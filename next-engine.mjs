...
// SPOKEN to-do — the first full sentence, capped on a clean word boundary. Board directionals
// run to three or four sentences of rationale; read aloud verbatim they pushed the briefing to
// 4.5 min and buried the imperative under its own justification. The tail is NOT dropped — the
// email HTML and .thetacog/next-routed.json both carry every to-do in full, which is where a
// to-do is acted on. Audio gets the actionable head; the artifact keeps the whole thing.
export const AUDIO_WPM = 176;          // scripts/next-briefing-send.sh NEXT_AUDIO_RATE default
const SPOKEN_TODO_WORDS = 28;
function spokenTodo(t) {
  let s = String(t || '').trim();
  const m = s.match(/^[\s\S]*?[.!?](?=\s+["'“(]?[A-Z0-9]|\s*$)/);
  if (m) s = m[0].trim();
  const w = s.split(/\s+/);
  if (w.length > SPOKEN_TODO_WORDS) s = w.slice(0, SPOKEN_TODO_WORDS).join(' ').replace(/[,;:—–-]$/, '') + '…';
  return s;
}
function renderNarration(R) {
  ...
  if (lead.todos.length) { N.push(`\nTop to-dos for ${say(lead.persona)}.`); lead.todos.slice(0, 5).forEach((t, i) => N.push(`${i + 1}. ${say(spokenTodo(t))}`)); }
