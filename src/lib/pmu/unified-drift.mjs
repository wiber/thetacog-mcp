...
export { COORDS };
import { WALK_KNOBS } from './walk-knobs.mjs';   // THE ONE WALK-KNOB HOME
...
// 2026-09-14 (operator: "the full is the minimum"): WALK_OPTS reads THE ONE HOME (src/lib/pmu/walk-knobs.mjs).
// The 2026-06-30 depth sweep below chose depth 2 as the shallowest depth that still discriminates; the
// panel, the commit receipt and the lens email have walked at depth 8 · topK 2 · budget 220 since
// fc3af13580, and a tape walked shallower than the receipt is a second instrument (io1b, 4,667 rows).
// Measured 2026-09-14 on four 800-char texts: depth 8 walks 17–19 cells (not a flood), σ 1.5–2.9, and
// fill stays far under SATURATION_FILL_PCT — the saturation guard below still stands as the tripwire.
export const WALK_OPTS = { maxDepth: WALK_KNOBS.maxDepth, topK: WALK_KNOBS.topK, budget: WALK_KNOBS.budget, budgetMs: WALK_KNOBS.budgetMs };
...
export const CHAT_WALK_MAX_DEPTH = WALK_KNOBS.maxDepth;   // the full walk is the floor; no env valve (a second home)
...
// ── buildSensor — the TARGET OVERRIDE seam (custom/support reefs ride THE ONE WALK) ─────────────────
// Before this, `targets`/SNIP_Z were module-level constants bound to the canonical LIB144, so any
// custom reef (per-room reef, foreign-repo bootstrap, a support domain) had to fork a SECOND placer —
// exactly the two-systems-diverge failure this module exists to kill. A sensor is { targets, snipZ,
// medianZ, source }: 144 snippet strings + their precomputed gzip lengths. Accepts a 144-array of
// strings, a raw lib array of { row, col, snippet|seed }, or a { anchors|nodes } wrapper — the same
// shapes LIB144 itself ships in. The APERTURE is part of the contract (META-BULK): gzip-NCD only
// measures meaning when both sides of the compression are the same size-order, so the sensor carries
// its median snippet gzip-mass and walkShape marks apertureRatio/apertureMismatch on every custom-
// sensor receipt — a thin reef can never silently hallucinate placements as if it were calibrated.
function medianOf(xs) { const s = xs.filter(Boolean).slice().sort((a, b) => a - b); return s.length ? s[Math.floor(s.length / 2)] : 0; }
export function buildSensor(lib, { source = 'custom' } = {}) {
  let arr = lib;
  if (arr && !Array.isArray(arr)) arr = arr.anchors || arr.nodes || [];
  if (!Array.isArray(arr)) throw new Error('buildSensor: lib must be an array (144 strings or {row,col,snippet} anchors)');
  let t;
  if (arr.length === 144 && arr.every(x => typeof x === 'string' || x == null)) {
    t = arr.map(x => x || '');
  } else {
    const slots = new Array(144).fill(null);
    for (const a of arr) { const r = aIdx(a.row), c = aIdx(a.col); if (r >= 0 && c >= 0) slots[r * 12 + c] = a; }
    t = slots.map(a => (a && (a.snippet || a.seed)) || '');
  }
  const snipZ = t.map(x => (x ? gzipLen(x) : 0));
  return { targets: t, snipZ, medianZ: medianOf(snipZ), source };
}
// the canonical sensor — the exact module-level constants the default path always used (byte-identical).
export const CANONICAL_SENSOR = { targets, snipZ: SNIP_Z, medianZ: medianOf(SNIP_Z), source: 'canonical' };
// aperture-parity band: a custom reef whose median snippet gzip-mass is >4× or <¼ the canonical
// library's is outside the size-order where gzip-NCD reads meaning (META-BULK) — marked, never hidden.
export const APERTURE_RATIO_BAND = [0.25, 4];

export function litScores(text, sensor = CANONICAL_SENSOR) {
  if (!text || !text.trim()) return new Array(144).fill(0);
  const docZ = gzipLen(text);
  return sensor.targets.map((t, i) => ncdSim(docZ, text, t, sensor.snipZ[i]));
}
...
//   targets    TARGET OVERRIDE: a custom reef for the seeding sensor — a buildSensor() result, or any
//              lib shape buildSensor accepts. Custom/support reefs ride THE SAME walk (never a forked
//              placer); the receipt carries sensorSource + apertureRatio/apertureMismatch (META-BULK:
//              a reef outside the canonical size-order is marked, never silently trusted). The walk
//              SUBSTRATE is already overridable via opts.gridBits (definerWalk144 passes it through).
...
export async function walkShape(text, { floor = SHAPE_FLOOR, scores = null, timeoutMs = 0, opts = WALK_OPTS, targets: targetsOverride = null } = {}) {
  const t0 = performance.now();
  // resolve the seeding sensor: canonical (byte-identical default) or the custom-reef override.
  const sensorLib = targetsOverride
    ? (targetsOverride.targets && targetsOverride.snipZ ? targetsOverride : buildSensor(targetsOverride))
    : CANONICAL_SENSOR;
  const apertureRatio = sensorLib.source === 'canonical' ? 1
    : +(sensorLib.medianZ / (CANONICAL_SENSOR.medianZ || 1)).toFixed(3);
  const apertureMismatch = apertureRatio < APERTURE_RATIO_BAND[0] || apertureRatio > APERTURE_RATIO_BAND[1];
  const sensorMark = { sensorSource: sensorLib.source, apertureRatio, apertureMismatch };
  ...
  const sc = scores || litScores(text, sensorLib);
  ...
  // BINARY ABSENT → UNMEASURED (operator 2026-09-14: "only the one same rust for all"). This used to compute a JS
  // gzip placement and a JS σ and label it 'gzip-fallback' — a second instrument painted as a degraded reading. A
  // reading the one binary did not produce is not a reading: no shape, no σ (the out-of-band sentinel), the reason.
  const fallback = (reason) => ({
    shape: new Set(), heat: new Array(144).fill(0), raw: null,
    sigma: UNPLACED_SIGMA,                                 // out-of-band sentinel — never a number a caller can mistake for drift
    coords: [], plies: 0, hops: 0, cells: 0, seed, seedCoords,   // the gzip seed ran (JS parity port) and is reported, never walked
    matrixCells: 0, fillPct: 0, walksPerSec: 0, saturated: false,
    seedGzipUs,
    sensor: 'unmeasured', unmeasured: true, fallback_reason: reason, ...sensorMark,
  ...
  const unplaced = (reason) => ({
    ...
    sensor: 'unplaced', unplaced: true, fallback_reason: reason, ...sensorMark,
    ms: +(performance.now() - t0).toFixed(2),
  });
...
export const COMMIT_WALK_DEPTH = WALK_KNOBS.maxDepth;   // the same home; no env valve
