...
// PMU_BINARY override lets a guard force the binary "absent" (→ marked gzip-fallback) without moving
// the real daemon. Default = the same on-chip binary definerWalk144 runs.
const DAEMON = process.env.PMU_BINARY || resolve(REPO_ROOT, '.thetacog/pmu/target/release/pmu-onchip');
