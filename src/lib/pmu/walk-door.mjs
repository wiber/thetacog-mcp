...
import { walkBudget, loadPerCoreNow } from './walk-budget.mjs';   // 2026-09-20: the door reads the load before it spawns — see walk-budget.mjs
...
import { appendFileSync, mkdirSync, existsSync, statSync, readFileSync, writeFileSync, unlinkSync } from 'node:fs';
...
// THE WALK LOCK (2026-09-20): engaged only above WALK_LOAD_SERIALIZE load per core. One file, the holder's pid inside; a
// holder that is dead, or older than WALK_LOCK_STALE_MS (no walk lives that long — the timeout is ≤ 2 × base), is overtaken.
// Sync by design: the door is sync, the wait is a bounded Atomics.wait, never a spin.
export const WALK_LOCK = process.env.PMU_WALK_LOCK || resolve(REPO, '.thetacog/walk.lock');
const WALK_LOCK_STALE_MS = 10000;
const sleepSync = (ms) => { try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms); } catch { /* no SAB: skip the wait, the retry still happens */ } };
function lockAcquire(path, now = Date.now()) {
  for (let pass = 0; pass < 2; pass++) {
    try { mkdirSync(dirname(path), { recursive: true }); writeFileSync(path, JSON.stringify({ pid: process.pid, at: now }), { flag: 'wx' }); return { held: true, holder: null }; } catch {}
    let l = null; try { l = JSON.parse(readFileSync(path, 'utf8')); } catch {}
    let alive = false; if (l && Number.isInteger(l.pid)) { try { process.kill(l.pid, 0); alive = true; } catch {} }
    if (alive && now - (Number(l.at) || 0) < WALK_LOCK_STALE_MS) return { held: false, holder: l.pid };
    try { unlinkSync(path); } catch {}   // dead or stale holder — overtaken, then one more wx attempt
  }
  return { held: false, holder: null };
}
function lockRelease(path) { try { const l = JSON.parse(readFileSync(path, 'utf8')); if (l && l.pid === process.pid) unlinkSync(path); } catch {} }
export function lensWalk(text, { source = 'turn', ref = null, bulk = null, repoRoot = REPO, timeoutMs = 800, seed = 'matched', tape = WALK_TAPE, extra = null, maxChars = null, digest = null, lane = null, knobs = WALK_KNOBS, session = process.env.LENS_SESSION_ID || process.env.CLAUDE_SESSION_ID || null, receiptsDir = undefined, before = null, seedAperture = SEED_APERTURE, room = null, reefPath = DEFAULT_REEF_PATH, axesPath = DEFAULT_AXES_PATH, loadPerCore = undefined, lockPath = WALK_LOCK, exec = execFileSync } = {}) {
  const bin = resolvePmuBinary(repoRoot);
  // THE BUDGET: read the load once, before anything is spawned (walk-budget.mjs). PMU_WALK_BUDGET=0 pins the old fixed door
  // for an A/B only — the default is the budget.
  const load = loadPerCore !== undefined ? loadPerCore : (process.env.PMU_WALK_BUDGET === '0' ? 0 : loadPerCoreNow());
  const budget = { ...walkBudget({ loadPerCore: load, base: timeoutMs }), deferred: false, waited: 0, holder: null };
  ...
  // C226d (2026-09-24): THE STAMP NAMES THE BYTES, NOT THE REQUEST. The cap is applied before the digest, so a requested 'strided' over
  // a source whose cap is at or under DIGEST_CHARS (spec-tree: cap 800 < 900) is a no-op — the binary received the head. Stamping 'd' on
  // those rows made every head-800 tree row read as a different instrument from a head-800 read (C226a's exact-id gate → UNMEASURED on
  // identical bytes; the tree carried t800b1dk8 beside t800b1hk8 for the same instrument). The request stays on the row as digest_requested.
  const digestRequested = mode; const digestEffective = used === String(text).slice(0, cap) ? 'head' : mode;
  ...
  if (process.env.PMU_BINARY === 'absent' || !bin || !existsSync(bin)) err = 'binary absent';
    ...
    // above the threshold: one walker at a time. Not had after one bounded wait and one retry → DEFERRED, nothing spawned.
    let held = !budget.serialize;
    if (budget.serialize) {
      let got = lockAcquire(lockPath);
      for (let r = 0; !got.held && r < budget.retries; r++) { sleepSync(budget.waitMs); budget.waited += budget.waitMs; got = lockAcquire(lockPath); }
      held = got.held; budget.holder = got.holder;
    }
    if (!held) { budget.deferred = true; err = `deferred: load ${budget.loadPerCore}/core — walk lock held by pid ${budget.holder ?? '?'} after ${budget.waited} ms (${budget.retries + 1} tries)`; }
    else {
      try {
        const j = JSON.parse(exec(bin, args, { encoding: 'utf8', timeout: budget.timeoutMs, maxBuffer: 8 * 1024 * 1024 }));
      ...
      finally { if (budget.serialize) lockRelease(lockPath); }
    }
  }
  ...
  const row = {
    ...
    aperture: { cap, used: used.length, bulk: bulkChars ? 1 : 0, digest: digestEffective, digest_requested: digestRequested, ladder: attempts.length, thread: thread ? 1 : 0 },
    ...
    aperture_id: `t${cap}b${bulkChars ? 1 : 0}${digestEffective === 'strided' ? 'd' : digestEffective === 'prose' ? 'p' : 'h'}k${knobs.maxDepth}`,
    ...
    sensor: admissible ? 'metal' : budget.deferred ? 'deferred' : 'unmeasured', admissible, attempts, err,   // 'metal' ONLY for a reading the seed's null test admits; 'deferred' when the load kept it from spawning (walk-budget.mjs); otherwise UNMEASURED with every attempt on the row
    budget,   // timeoutMs · loadPerCore · serialize · deferred · waited — the door's own setting, on every row
