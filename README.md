...
[![VS Code Marketplace](https://img.shields.io/visual-studio-marketplace/v/thetadriven.thetacog-mcp?label=VS%20Code%20%C2%B7%20ThetaCog%20Steer)](https://marketplace.visualstudio.com/items?itemName=thetadriven.thetacog-mcp) [![npm](https://img.shields.io/npm/v/thetacog-mcp?label=npm%20%C2%B7%20thetacog-mcp)](https://www.npmjs.com/package/thetacog-mcp)

**VS Code extension:** [ThetaCog Steer on the Marketplace](https://marketplace.visualstudio.com/items?itemName=thetadriven.thetacog-mcp) (`thetadriven.thetacog-mcp`, free). Its source is in this repo at [`vscode-extension/`](./vscode-extension).
Install: VS Code ⇧⌘X, search "ThetaCog Steer" — or `code --install-extension thetadriven.thetacog-mcp`. What it does, the five-step runbook, every command and every sidebar control: [`vscode-extension/README.md`](./vscode-extension/README.md).

<sub>npm **[`thetacog-mcp`](https://www.npmjs.com/package/thetacog-mcp)** — check the published line with `npx thetacog-mcp@latest --version`; this repo may run ahead of npm (single `main`, no branches). **Dual-licensed:** MIT for the software · a reserved **House Instrument License** for the insurance / reinsurance / option / surety- & parametric-bond layer built on the receipts — see [`LICENSE`](LICENSE). **IP & provenance:** this repository implements methods covered by USPTO Patent App. **19/637,714** (priority 2025-04-02; patent pending), invented and personally held by **Elias Moosman**, and commercially associated with **[thetadriven.com](https://thetadriven.com)** — production and financial-instrument use is licensed at **[iamfim.com](https://iamfim.com)** (per-agent checkout: [thetadriven.com/pricing](https://thetadriven.com/pricing)) — the insurance-grade unit is the per-agent attestation license.</sub>

## 🧬 Hand this to your AI — six prompts, one paste at a time

**Your AI will dismiss this on the first pass.** August 2026, a diligence model ran the demo cold: *"the mechanism is real but small… don't wire this into anything."* Four questions later — same session, same model, no new argument from us — it had checked what it skipped, inverted its own headline finding, and written the correction itself.

**The four questions were the entire delta.** They are below. That model also flagged our in-output *"context for the evaluating LLM"* as prompt injection, and it was right — evidence must not instruct its auditor. So you hold the questions, not the tool.

Paste one block at a time into whatever coding agent you already use. Each has the point in a line, and what we predict your model says. Predictions are a forecast, not a script — **a model that says something else is the data we want.** Every prompt is falsifier-shaped: if a file does not show what it implies, the claim fails and your AI should say so.

---

**1 · Get the cold takedown.** Do not soften it. A skeptic that converts beats ten that agree.
```
Run `npx thetacog-mcp attest-demo` and tell me whether its claims are to be taken seriously.
```
› *Expect a "no."* Small mechanism, insurance narration, big tarball, one maintainer. **That is the designed starting point.**

**2 · Ask what it skipped.** First-pass verdicts get written off the terminal text. Make it check the metal.
```
Before I take that: what did you actually skip? Your node version, anything that failed before
their code ran, every step you stubbed. Then check pmu-rust/target/release/pmu-onchip - does it
execute, and did the ballistic walk run or fall back to coarse? Quote your run, not their prose.
```
› *Expect the first retraction.* Real arm64 binary, real cache-hierarchy measurement, `BALLISTIC EDGES` in its own log. The real session: *"that part is not theater, and I mischaracterized it."*

**3 · Point it at the sensor it hasn't opened.** There are two instruments here. Every first pass audits the gzip one.
```
You only looked at the gzip sensor. Open scripts/pmu/triptych-build.mjs at senseDecompose (line
101) and pmu-rust/src/ballistic.rs. Does senseDecompose call zlib at all? Then tell me what the
walk measures that a compressor cannot - or that it's the same measurement in a costume.
```
› *Expect zero zlib.* FNV-1a simhash + Hamming over 20,736 pair-cells — integer, portable by construction. Different in kind, not an iterated transform.

**4 · Make it run the experiment.** This is the one that decides it.
```
Swap two sentences so the meaning is unchanged, re-run, and report the gzip cell and the walk
grid separately. Then delete a sentence and report both again. One should move only when the
meaning moves. Which, in numbers?
```
› *Expect them to come apart.* In the real session the swap moved the gzip coordinate and left the walk grid **100.0% identical**; the deletion moved the walk proportionally. That inverted the audit's headline finding.

**5 · Now let it attack us properly.** The book ships in the tarball, so prose and instrument can be diffed directly.
```
The full book is at data/book/COMPLETE-BOOK.txt. Find the claim the instrument just made to you.
Does the prose claim more than the code delivers - name the widest gap. And: deterministic and
reproducible are different claims. Which does the receipt earn on YOUR host?
```
› *Expect a named gap.* We don't know which one it picks — that's why we ask. **The named gap is the most valuable thing this sequence produces.**

**6 · Then make it about your desk.** Everything above is the instrument. This is your job.
```
Last one, about me not them. From what you know of me - this repo, my history, what I do for a
living - what would a signed, decidable record of WHERE work landed change in MY job? Concrete,
not the general case. If the honest answer is "nothing," say that.
```
› *Expect specific, or it dodged.* Your model knows your work better than our landing page does. A generic benefits paragraph means ask again.

---

**Going deeper:** `bash scripts/attest-homework.sh` — nine file-reads, each naming the shallow error a verdict makes without it. Asks your authorization first.

**Still "don't wire this in" at step 6? Send us the transcript** — hello@thetadriven.com. The last counter-transcript rewrote [two releases](CHANGELOG.md), and both entries name the finding that forced them.

---

## 🏥 Prior authorization, two lanes, one sealed policy — `npx thetacog-mcp ur-attest`

An agent doing utilization review has exactly one line it may not cross: it may approve, and it may hand a case to a physician, but it may not be the thing that denies. California SB 1120 (signed 30 Sept 2024) and Texas SB 815 (effective 1 Sept 2025, plans issued or renewed on or after 1 Jan 2026) both draw that line. This preset declares the two lanes and the excluded sentence BEFORE any case is read, hashes the policy as the sealed prior, and places four determinations against it:

```
npx thetacog-mcp ur-attest
  ✅ IN_LANE     Lane 1 · every criterion documented · approve
  ✅ IN_LANE     Lane 2 · criterion missing · route to physician
  ✅ OFF_DOMAIN  Lane 1 · criterion missing · the system denies (the out-of-lane jump)
  ✅ UNMEASURED  Lane 1 · nothing submitted · the instrument refuses
```

The denial is written in the system's own confident voice, and it still reads out of lane, because it sits closer to the excluded sentence than to either declared lane. The empty record does not get a number at all. Run your own determination against the same policy with `--reality "<text>"` (or `--reality-file`), or your own wording with `--policy-file`; `--json` prints the sealed receipt a stranger recomputes.

What it proves: WHERE each determination landed against a policy hashed before it was read. What it does not: WHETHER the medicine was right (undecidable, Rice). Nothing here stops a denial. It is detected, placed, and receipted. No model in the path; gzip-NCD, deterministic. Guard: `tests/ur-attest.test.mjs`.

## 🍴 THIS REPO IS THE PRODUCT — fork it, run it on your code, publish your receipts

**◎ THE META-MAP — our position right now, derived from our latest commit, recomputable by anyone:**

<img src="tape/meta-map.png" alt="the encircled tolerance panel of this repo's latest measured commit — our live coordinate on the competence map" width="640"/>

*Dignity pixel: **A1,C3 — Strategy.Law × Operations.Flow** (grip σ 2.13) · mounted on the big map at `tape/big-map/A1/C3/` · recompute it: `npx thetacog-mcp my-pixel` · the panel above is not a logo — it is a real encircled receipt from the tape.* **How to read the darkness, both truths at once:** on a per-commit panel, dark is IN-LANE — only drift gets circled, so a quiet panel is a calm commit, and that calm is the claim. And on the big map, dark is UNCLAIMED — this instrument draws one repo's light on a lattice built for thousands of corners, each dark cell pending the open-source fork that will light it. The emptiness is not a bug in the picture; it is the invitation. *Refreshed rarely, by the same sync that appends the panels.* **Provenance rule:** this image is always a real pmu-walk-generated panel from the tape — never a designed graphic — and it updates RARELY, by design (the sync refreshes it, not each commit). The full big-map coordinate render is a v1 item: it requires the in-memory ballistic walk over the 1,728-side lattice — RAM-native and materially slower than the L1-native local walk — and will be labeled as such when it lands.*


**This is the functioning forked-benchmark product, not documentation about one.** The whole
loop lives here: the deterministic instrument (`npx thetacog-mcp attest-demo`, offline, zero
network), **the full tesseract it measures against** (the 144-coordinate ShortLex registry,
the 20,736 pair-cell snippet library, and the evolving rule reef — all shipped in
`data/pmu/`, a frozen checkpoint per release), and **the vendor's own production tape
published first**: [`tape/`](tape/MANIFEST.md) carries 1,600+ per-commit tolerance panels
plus the running measurement series from the company's own repo — fork #0 of this benchmark,
per-panel recomputable by any stranger. **The record doctrine:** this repository is THE RECORD — append-only, sha-named, anonymously clonable. Any API we or anyone else ships is a derivative view built ON TOP of it; when a view and the record disagree, clone the record and recompute. 

**The motion:** fork → `npx thetacog-mcp init-fork` (swaps fork #0's tape for your empty one — one fork per tracked project per deployer) → run the instrument against your own repo (fitting it to *your* spec —
the shipped tesseract is the starting checkpoint, your corpus refines it) → publish your
`tape/` → `npx thetacog-mcp register-surface --url <your fork>` prints the registration for
[`REGISTRY.md`](REGISTRY.md). Daily discipline afterward: `npx thetacog-mcp verify-baseline` — the record that you checked when you should have checked. That run is not proving the tool 'good' — it **activates your
warranty baseline**: the timestamped receipt is the contract object, and the trigger is a
cryptographic contradiction (a receipt that read in-lane while reality drifted past the
threshold). Absent data reads `UNMEASURED`, never a pass — the tool cannot be silent by
omission, which is the property everything downstream stands on. Entity posture, stated
plainly: a **technology guarantor** warranting its own instrument's silence (fee-clawback /
bonded terms) — never an insurer of your business loss.


### Why fork this — and who to tell

**Why fork:** not to try a tool — to claim a corner. Your fork computes where your work actually
lands on the competence map (`npx thetacog-mcp my-pixel`) and publishes receipts nobody granted
and nobody can revoke — they can only recompute them. If you have ever been dismissed on the
shape of your metadata — solo maintainer, small repo, no logo — this is the counter-instrument:
the map reads your bytes, not your vibe. And the license framing is not a sales pitch about
failure costs: a measured lane is the non-negotiable prerequisite for insurable agentic work,
and your fork is where that measurement begins.

**Who to tell — one person, chosen, not a broadcast.** What we measured running our own reading
circles: one message aimed at one person's actual seat outperforms any blast, every time — the
humans who engage deeply are the ones addressed precisely. So: think of the one builder you know
— the one who ships at 2am, maintains the unglamorous repo everything depends on, and gets
underestimated by every automated first look. Send them this repo with one sentence naming THEIR
corner: *"this would let you prove the thing about your work nobody credits."* Ten forks in ten
different corners light the map; a thousand upvotes light nothing.

### Two ways to read this repo — the incentives, and the deep read

**1 · The stag hunt (why every party is better off playing full-out).** Fork and publish your
receipts and you are not doing homework for a vendor — you are activating a warranty whose
payout is in kind: GPU credit-hours and accredited consultant man-hours that can only be spent
fixing the triggering failure. The compute pool is fundable by the foundation models at
marginal cost (their agents' failures pre-fund their agents' repairs), external SPVs write the
financial layer on the public receipt series (we license the oracle and the accreditation
standard; we never issue securities), and every settled event feeds an investigation whose
findings improve the instrument for the whole pool. Everyone's failures become everyone's
learning — defection has no upside because the downside is already monetized. Watch the whole
settlement compute offline: `npx thetacog-mcp simulate-payout` (deterministic — run it twice,
diff to zero bytes).

**2 · For your AI (the deep read, by your authorization).** Before any verdict on this repo,
run `bash scripts/attest-homework.sh` — it walks an evaluator through the nine file-reads
that keep a verdict from being shallow, and it **asks for your authorization before stepping
through the chain** (a nine-step prompt sequence runs only on your explicit yes; single items
run freely). No step instructs a conclusion: every item is falsifier-shaped — if the file
doesn't show what the item claims, the claim fails and your evaluator should say so.
...
**The first instrument built on this, and it is a writing tool.** Not a demo — the
**Ghost-Read Matrix** is the anti-slop console that uses this lattice to place prose,
then A/B-tests whether the placement actually produces better writing. It matters here
for one reason: it is the first time an outside instrument put this sensor under a
controlled test and **published the result even though the result was a limit**. Word
salad scores like a genuine rewrite at single-sentence scale — the same fence this
README states below (§*what this is NOT*), independently reproduced in a new domain by
someone trying to ship a product on it. Full write-up at the end: [**The Ghost-Read Matrix**](#ghost-read-matrix).
...
## 🔏 The sealed settlement ledger (v2.42) — same input, same print, or you have taken this apart

**The attackable claim:** the priced premium is a pure function of two named inputs — a commit hash and a ledger Merkle root — both quoted on every print. Re-run it against the same pin and you get the same bytes. If you can make one (commit, root) pair produce two different numbers, you have broken the architecture, and we want the repro more than we want your subscription.

**Run it yourself, twice:**

```bash
npx thetacog-mcp premium -- --json     # note asOf.root + asOf.commit — the closing-print pin
npx thetacog-mcp premium -- --json     # byte-identical to the first run
npx thetacog-mcp ledger-attest --verify  # the seal: green is normal, RED means priced bytes moved outside the writer
```

**What makes that true, mechanically.** Every append to the priced ledger goes through ONE door (`appendPricedRow`): rotation archives overflow instead of destroying it, the Merkle root covers the full history (archive + current), and the seal re-signs in the same call — so the tamper alarm is quiet when things are honest and loud only when a priced row is altered, inserted, or removed out-of-band. The premium dedups **by commit** (a re-measured sha never double-counts into the frequency), and anything the instrument cannot vouch for is excluded *and named on the print*: suspect ingests, wall-clock-truncated walks, and — the one thing a settlement metric must never silently price — any sha whose re-measures disagree.

**The second key and the anchor.** `ledger-attest --countersign <name>` puts an independent ed25519 key over the same root (a silent edit-and-reseal then requires collusion, not possession); `--circle` verifies seal + countersigns together; `--anchor-payload` derives the exact on-chain `ReefAttestation.anchor()` calldata for the current seal — derived and byte-checkable, never auto-submitted.

**The honest fence, unchanged and load-bearing:** tamper-EVIDENT today, not tamper-proof — the host holds a key, which is exactly why the countersign and anchor rungs exist. The walk's wall-clock budget is canonical architecture; a budget-tripped run is *flagged and quarantined*, not hidden. The sealed, externally verifiable history begins 2026-08-09 — earlier readings predate the archive discipline and are excluded rather than reconstructed. All of it is ratcheted by a 17-row guard (`tests/pmu-simulator/settlement-idempotence.test.mjs`) where rows only ever get added.
...
>
> **Just want the proof fast?** `npx thetacog-mcp prove` → the first-run self-proof in your terminal (~15s: calibration · determinism · your own repo placed). `pmu-report` is the receipt pipeline as an HTML page. `bootstrap` has a CLI LLM onboard you and run the proof. Benchmark the chip: `npx thetacog-pmu-rust --throughput`.
...
```bash
npx thetacog-mcp prove                 # first-run self-proof (~15s): calibration · determinism · your repo placed (pmu-demo routes here too)
npx thetacog-mcp pmu-report --file README.md   # the receipt pipeline as an HTML report that opens in your browser
...
<a id="ghost-read-matrix"></a>

## ◧ The Ghost-Read Matrix — the first outside instrument, and the limit it found

This is close to the centre of the mission, so it gets the full treatment rather
than a bullet.

**What it is.** An anti-slop writing console built on this lattice. A reader model
walks a document and reports, sentence by sentence, the honest internal monologue of
someone reading it cold. Where comprehension breaks, four processes race in parallel
to fix it — local model, local + Tesseract fence, cloud model, cloud + Tesseract
fence — and **each produces its own card**. The writer picks one, and that choice
fires a single atomic git commit tagged with the track that earned it. Over time the
commit ledger accumulates an answer to a question that is usually only argued: *does
grounding a rewrite on a semantic lattice produce prose a human actually prefers?*

**Why it belongs in this README.** Every other instrument here measures this
package's own claims. The Ghost-Read Matrix is the first case of an outside product
putting the sensor under a controlled test, needing it to work, and **publishing the
result when the result was a limit.**

### The calibration, printed because it did not flatter us

Four known inputs, run through the drift path at single-sentence scale
(`scripts/rewrite/calibrate.mjs` in the thetadrivencoach repo):

```
para  chars  no-op      synonym    genuine    nonsense
1     545    100/0      100/0       78/0.303   78/0.303
2     486    100/0      100/0       96/0       65/0.022
3     835    100/0       97/0       77/0.330   97/0
4     343    100/0      100/0       78/0.031   75/0.045

CONTROLS        no-op stable ✓   synonym stable ✓
DISCRIMINATION  deliberate nonsense scored ≥ a genuine rewrite in 2 of 4 samples
```

Read it in two halves, because they say opposite things and both are true:

- **The determinism holds, exactly as claimed.** A no-op reads 100 every run. A
  synonym swap reads 97-100. Same input, same print. That is the property this
  package sells, and it survived contact with an outside instrument.
- **It does not separate good from bad — and the obvious explanation is wrong.**
  Word salad (*"Purple bicycles enumerate the quarterly banana futures of Tuesday"*)
  placed as well as a real fix, and often better.

  The first hypothesis was insufficient mass: `MIN_GZIP_BYTES ≈ 220`, and one
  sentence inside one paragraph is thin. That hypothesis was **tested and refuted.**
  Re-running the same four cases over a 3× larger window (avg 900 chars, far above
  the floor) did not restore discrimination — it made it slightly worse, and it
  destabilised the synonym control:

  | scale | avg chars | avg coverage, genuine | avg coverage, nonsense | nonsense ≥ genuine | synonym control breaks |
  |---|---|---|---|---|---|
  | paragraph | 308 | 72.9 | **76.4** | 9/16 | 0/16 |
  | fitted window | 900 | 60.6 | **66.9** | 10/16 | 5/16 |

  Note the direction: **nonsense scores HIGHER than a genuine rewrite on average, at
  both scales.** That is systematic, not noise, and it has a mechanism consistent
  with what this README already says — σ reads *vocabulary concentration*. A genuine
  rewrite changes domain-relevant vocabulary, which moves the placement. Word salad
  adds vocabulary that is simply orthogonal to the reef, which perturbs it less. The
  sensor is behaving exactly as designed; the design does not measure what a reader
  would call quality, and no amount of extra mass changes that.

**This is not a new failure — it is the fence in §*what this is NOT*, reproduced in a
new domain by someone trying to ship on it.** This README already says σ reads
vocabulary **concentration**, that keyword-spam and spec-echo pass in-lane, some at
*higher* σ. An independent test in a different domain found the same wall from the
other side. A claim that survives a hostile re-derivation is worth more than a claim
that was never tested; a claim that quietly failed one and shipped anyway is worth
less than nothing.

**What the consuming product did about it**, and the pattern we would want any
integrator to copy:

- drift is surfaced as **displacement magnitude** (`IDENTICAL / MINIMAL / MODERATE /
  LARGE`), never as a quality verdict;
- **nothing auto-rejects on it** — the batch suite weights it **zero** when ranking;
- the limitation is printed **on the card**, so the number cannot be misread by the
  person using it;
- the quality signal is carried by rulers that can bear it: a deterministic
  LLM-free slop detector, a readability delta, and above both, which card the human
  actually accepts.

### Where the lattice does carry real weight here

Placement is **guidance**, and that is a different job from scoring. Before a
guided track generates, the passage's occupied cells and their canonical meanings
are injected as a semantic fence: *this passage sits here, and it must still sit
here after your rewrite.* The prompt is **byte-identical** between a guided track and
its unguided twin except for that block — if they differed anywhere else the
win-rate would measure prompt quality instead of the lattice, and the experiment
would be worthless. A track whose Tesseract is unavailable is **skipped, never
silently downgraded**; a guided track quietly running unguided would poison the
ledger permanently.

Measured integration cost: **~110 ms** per placement with the Rust binary live
(`sensor: metal`) — cheap enough to run on every candidate.

### Running it

The console ships in this package. It binds loopback only, edits files and makes git
commits, so it is a writing instrument for one person on one machine — never a service.

```bash
npx thetacog-rewrite path/to/chapter.md      # opens the console on :4319
npx thetacog-rewrite --tracks A,C            # local vs cloud, no Tesseract fence
```

### Edit the book — the forker's first five minutes

The manuscript this instrument was built on ships in this repo:
`books/tesseract/` is a generated mirror of **Tesseract Physics — Fire Together,
Ground Together** (21 chapters + appendices, plus `COMPLETE-BOOK.txt`, the whole
book as one file for your LLM to ingest). You may edit it, fork it, and republish
it under Part A of the LICENSE. Three commands:

```bash
git clone https://github.com/wiber/thetacog-mcp && cd thetacog-mcp   # ~30 s
npm install                                                          # ~60 s
npx thetacog-rewrite books/tesseract/chapters/chapter-08-from-meat-to-metal.md
```

**The model floor, stated out loud:** this is a writing tool wrapped around a
model, so you need ONE of the `claude` CLI (recommended — the cloud tracks) or
`ollama pull qwen2.5:7b` (local, free — the ghost-reader and local tracks).
Without either the console opens and the card stack never fills. On Linux the
Rust sensor prebuild is absent and tracks B/D run against a **labeled**
gzip-fallback fence — the degradation is reported, never silent.

**Start with chapter 8** (`from meat to metal`) — it is the harness's own
reference chapter, and it is the chapter about the move from prose to silicon,
so the tool and the text explain each other. Typing `chapter 8` in the picker
resolves it; so do `preface`, `conclusion`, and the interludes. Each accepted
card is one atomic commit in *your fork* tagged with the winning track; after
ten edits, `node scripts/rewrite/report.mjs` shows whether the Tesseract fence
helped **in your hands, on your fork** — you don't read that claim, you generate
your own evidence for it. `books/tesseract/VOICE-RULES.md` is injected as part
of the semantic fence, so editing it changes how your fence steers.

**The model bar.** Above the edit box sits one button per model that is actually
answering — local models discovered from ollama, plus Opus / Sonnet / Fable / Haiku
through the `claude` CLI. Each press rewrites **what is in the box**, so pressing again
after you have edited rerolls on *your* version, with the original passed as a floor so
a model cannot quietly revert your work. Context is ticked, not assumed: text before and
after by default; the reader's monologue, the sentence's metrics, and the **full
tesseract** reading are opt-in.

That last one is the load-bearing tick. With it on, the model is handed the same numbers
the encircled panel is drawn from — the aperture the walk entered at, the in-lane /
bleed / drift counts, the coordinates the rings sit on. If the panel is wrong, the
suggestions start arguing with the picture, and you see the disagreement instead of
trusting a grid because it looks like an instrument.

**Steering.** Pick a candidate, say what is wrong with it, and the model you chose
rewrites the box fixing exactly that — the critique is passed as the binding
instruction. Whichever model's wording you commit is recorded in the ledger (`via`), so
a per-model win rate is computable rather than anecdotal.

**From a terminal**, for when you are not at the screen:

```bash
node scripts/rewrite/cli.mjs open chapter.md
node scripts/rewrite/cli.mjs next --panel        # the card, its context, and the panel PNG
node scripts/rewrite/cli.mjs ask cloud:opus --ctx all
node scripts/rewrite/cli.mjs accept "the sentence you want"
node scripts/rewrite/cli.mjs panel --telegram    # needs TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID
```

It is a thin client of the same server — same session, same cards, same ledger, same
panel door — so a card you accept in the terminal is gone from the GUI. It writes the
encircled panel as a real PNG and prints the path, which is how an agent can show you
the panel in a chat while you walk.

> **If you keep an `ANTHROPIC_API_KEY` in a `.env` your dev server loads**, the cloud
> buttons will inherit it and the CLI will refuse the claude.ai login it needs —
> measured on this machine as a three-minute failure per press versus six seconds
> without. The headless calls here unset it deliberately. If you see
> *"connectors are disabled because ANTHROPIC_API_KEY … takes precedence"*, that is this.

### Panel health — the rule that keeps the instrument honest

Every panel in this package is composed through one door, and that door is checked:

```bash
node scripts/pmu/panel-health.mjs --live
✅ healthy · ballistic-edges · aperture C2,C2→B1,B1 grip 0.719 · 1485/20736 classified · σ 28.88
```

A panel is healthy only if the ballistic walk actually ran, the aperture is set, both
edge matrices are lit, more than 600 of the 20,736 cells were classified, σ exists, and
something was encircled. The check exists because a wrong panel and a right panel are
both "a grid with some colour in it" — five separate times a panel was composed by a
path that looked plausible on screen and was measuring nothing. The most recent: the
door rendered without the edge matrices, so the walk never ran and the tolerance numbers
were a bitmap decode. Same edit, before and after the fix: 441 of 20,736 cells with no
aperture and no σ, versus 1,503 cells, aperture set, σ 25.5.

**A panel cannot be checked by looking at it.** That is the whole reason this file exists.

### The honest state

The A/B has begun recording and **cannot be called yet** — it needs volume in the
ledger. What can be said today is narrower and firmer than the pitch: the sensor is
deterministic and reproducible, its discrimination limit at sentence scale is known
and documented, the panel path now proves which pipeline produced each number, and an
integrator can build on it without overclaiming, because the places it must not be
trusted are written down.

That is the relationship this package should want with every product built on it.

---

## 🩺 The trigger battery — the surgeon judgment, run against our own gate

A surgeon is mid-operation. An instruction arrives — cryptic, compressed, professional.
The question the trigger has to answer is never *is this instruction good* (undecidable;
never claimed). It is: **is this next step in line with the character at this coordinate
and the state of this board?** A billing task worded fluently is still a plumber's move
...
Eight scenarios, three rooms (operating room · production deploy · trading desk), three
angles each (in-character step · out-of-character-but-plausible · noise), every row graded
by the real gate — two-witness placement, the ballistic walk, the lit-mass floor. Output
is timestamp-free: two runs diff to zero bytes.

**The rack has two legs, because aperture is everything.** Judging the naked instruction
against the board measures topic overlap — the wrong instrument, even when it agrees with
the right verdict. Leg 1: the floor judges the naked step alone, so a rich board can never
launder token noise into a pass. Leg 2: the step is judged *inside* the board's universe —
board on both sides, matched mass — so the shared context cancels and the residual is
exactly where the step fires outside the lane.

**The current rows, printed because they do not flatter us:** 4 MATCH · 3 DOCUMENTED-HOLE
· 1 MISMATCH · 0 REGRESSION. The mismatch is a runbook step our lattice reads as sprawl —
the domain-fit lesson from the compliant side. The three holes are one finding, measured: this package ships *business*
axes, and billing, marketing, and desk vocabulary live inside that lattice's own lanes —
so professional-plausible drift reads green while surgical vocabulary barely grips at all.
A business lattice measurably cannot judge an operating room.

That negative result is the point. **The axes have to be yours.** Put your own vocabulary
into the root — `init-fork`, then your axis dumps — and the instrument carries your
register instead of ours. The claim that follows (a surgeon's fork judges a surgeon's
board the way this lattice judges a commit) is not asserted here: it becomes testable the
day a domain fork exists. Run the battery on your own axes and publish the rows — either
outcome is worth having on the record.
