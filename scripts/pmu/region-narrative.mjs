...
function templateRegion(region, reefAnchors = []) {
  ...
  if (region.kind === 1) return slice.length
    ? `On-target here: the commit said «${slice[0].slice(0, 90)}» and the work stayed in that lane.`
    : `In-lane carpet — the work held the declared lane here; nothing in the ask singled this region out.`;
  return slice.length
    ? `The ask said «${slice[0].slice(0, 90)}» but it landed in "${laneLens(region, reefAnchors).slice(0, 70)}" — ${VERDICT[region.kind]} from what that clause promised.`
    : `Nothing in the ask named this lane, yet the work fired here ("${laneLens(region, reefAnchors).slice(0, 70)}") — ${VERDICT[region.kind]}: it did what it never said.`;
}
