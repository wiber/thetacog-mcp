...
import { TOL_RGB } from './annotate-regions.mjs';   // C18: the tolerance palette has ONE home
import { sliceMessageToRegions, loadPairLib } from './region-message.mjs';
import { coordGist } from './lattice-meaning.mjs';
import { execSync } from 'node:child_process';
import { llmPauseActive } from '../cog/llm-pause.mjs';   // C292: the one gate every automatic model caller asks
...
// WHOLE-PROCESS budget (2026-07-25). QWEN_TIMEOUT_MS is PER CALL; narrateRegions loops `cap`
// regions sequentially, so the per-call ceiling multiplies: 8 regions × 45s = 6 MINUTES per
// process. Stacked behind one serialized ollama that became 12-minute runtimes and a 10-deep
// pile-up (load 100, 29GB swap, 4.9GB wired). The budget is the ceiling the per-call timeout
// never was: once spent, remaining regions take the deterministic template fallback — the
// narrative still completes, it just stops paying the LLM. Never unbounded.
const NARRATE_BUDGET_MS = Number(process.env.NARRATE_TOTAL_BUDGET_MS) || 90000;
const NARRATE_T0 = Date.now();
export const budgetSpent = () => Date.now() - NARRATE_T0 >= NARRATE_BUDGET_MS;

// C292 — this runs from the post-commit hook, an AUTOMATIC caller: it asks the one gate (llm-pause.mjs) once per process and takes
// the template fallback while the ollama lane is held (active session, the pause file, or a bench holding the heavy lock).
// LLM_ACTIVE_OK=1 is the explicit ask (--demo by hand), which the gate never holds.
let GATE; const gateHeld = () => (GATE === undefined ? (GATE = llmPauseActive(undefined, { lane: 'ollama', explicit: process.env.LLM_ACTIVE_OK === '1' })) : GATE);
export const narrateGate = () => gateHeld();
async function callQwen(prompt) {
  if (budgetSpent() || gateHeld()) return null;   // budget gone or lane held → template fallback, no new LLM call
  try {
...
function synthDriftPanel() {
  ...
  const N = 144, B = 12; const G = TOL_RGB.green, R = TOL_RGB.red;   // C18: one home
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (fromFile) {
    // SINGLE-FLIGHT (2026-07-25). post-commit spawns this detached per commit with NO concurrency
    // cap, and every instance funnels into one ollama that serializes. Ten commits = ten processes
    // each waiting minutes on the same wedged queue. Atomic mkdir is the lock (EEXIST is the
    // contended branch); a stale lock whose PID is dead is reclaimed. Drop-if-busy, not a queue:
    // each narrative is per-SHA and superseded within minutes, so stacking them buys nothing and
    // costs the pile-up. The in-flight one finishes; this one exits clean.
    const lockDir = resolve(REPO, '.thetacog/cache/region-narrative.lock');
    mkdirSync(resolve(REPO, '.thetacog/cache'), { recursive: true });
    let held = false;
    for (const attempt of [1, 2]) {
      try { mkdirSync(lockDir); writeFileSync(resolve(lockDir, 'pid'), String(process.pid)); held = true; break; }
      catch (e) {
        if (e.code !== 'EEXIST') throw e;
        if (attempt === 2) break;
        let alive = false;                                   // reclaim a lock whose owner died
        try { process.kill(Number(readFileSync(resolve(lockDir, 'pid'), 'utf8').trim()), 0); alive = true; } catch { /* dead or unreadable */ }
        if (alive) break;
        try { rmSync(lockDir, { recursive: true, force: true }); } catch { /* raced */ }
      }
    }
    if (!held) {
      console.log('region-narrative → SKIPPED (another narrative in flight) · single-flight lock held');
      process.exit(0);
    }
    const release = () => { try { rmSync(lockDir, { recursive: true, force: true }); } catch { /* */ } };
