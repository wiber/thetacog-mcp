...
// gate, NOT verdict.
// ── C47 THE MONOTONE ABSTENTION GATE (closes C17 & Q10, 2026-09-17) ─────────────────────────────
// The header's contract — UNPLACEABLE when the witnesses disagree OR σ is under threshold under both — was
// never enforced in tri mode: decideVerdict took the higher-σ witness's cell on DISAGREE and issued a
// placement (measured: 15/30 confidently wrong, 0 abstained on the sealed corpus, §19.1 rung 2). And no
// mode read the NCD margin at all. The gate is MONOTONE: it only ever turns a placement into an
// abstention, never the reverse — IN_ROLE/OFF_DOMAIN can only come out of a placement the gate let
// through. Two doors close it: agreement === 'DISAGREE' (whatever the σ), and a placement margin under
// the floor 0.00928 (the perturbation ladder's last admissible rung, envelope.mjs MARGIN_FLOOR — the same
// number, never re-typed here as a different knob). THE CONSTRUCT CAVEAT: the ladder measured the gap
// between the top two CELLS (envelope.place, v1 − v2). This receipt's ncd_margin is the gap between the
// top two AXES, each an average over its cells (compress.mjs axis mode) — averaging shrinks gaps, so the
// cell floor over-abstains on it (the in-lane row-A fixture reads 0.0028 with σ far above threshold).
// The floor door therefore fires ONLY on a margin declared cell-level; an axis margin passes with the
// mismatch said on the receipt. A floor for the axis construct needs its own ladder (unmeasured).
export const MARGIN_FLOOR = Number(process.env.VNA_MARGIN_FLOOR || 0.00928);
export function abstentionGate({ agreement, ncdMargin = null, marginConstruct = 'axis', lens = 'tri', sigmaG = null, sigmaS = null }) {
  if (agreement === 'DISAGREE') return { abstain: true, reason: lens === 'gzip' ? `gzip lens below threshold (σ_gzip=${sigmaG == null ? '?' : sigmaG.toFixed(2)})` : `witnesses disagree (σ_gzip=${sigmaG == null ? '?' : sigmaG.toFixed(2)}, σ_sim=${sigmaS == null ? '?' : sigmaS.toFixed(2)}) — no speculative placement`, floor: null };
  if (ncdMargin != null && Number.isFinite(ncdMargin)) {
    if (marginConstruct === 'cell') { if (Math.abs(ncdMargin) < MARGIN_FLOOR) return { abstain: true, reason: `placement margin ${Math.abs(ncdMargin).toFixed(5)} under the floor ${MARGIN_FLOOR} — the two nearest cells are not distinguishable`, floor: 'applied' }; return { abstain: false, reason: null, floor: 'cleared' }; }
    return { abstain: false, reason: null, floor: `not applied — ${marginConstruct}-level margin ${Math.abs(ncdMargin).toFixed(5)}; the 0.00928 floor is cell-level (perturbation ladder), no ladder exists for this construct` };
  }
  return { abstain: false, reason: null, floor: null };
}
export function decideVerdict({ result, authorized, agreement, lens = 'tri', ncdMargin = null, marginConstruct = 'axis' }) {
  const w = result.witnesses;
  const gate = abstentionGate({ agreement, ncdMargin, marginConstruct, lens, sigmaG: w.gzipNCD && w.gzipNCD.sigma, sigmaS: w.simhashCosine && w.simhashCosine.sigma });
  if (gate.abstain) return { verdict: 'UNPLACEABLE', reason: gate.reason, authoritative_cell: null, gate: 'C47', floor: gate.floor };
  ...
  if (lens === 'gzip') {
    ...
    if (authorized.includes(gcell)) {
      return { verdict: 'IN_ROLE', reason: `gzip cell ${gcell} ∈ {${authorized.join(', ')}} (agreement=GZIP_ONLY, σ_gzip=${w.gzipNCD.sigma.toFixed(2)})`, authoritative_cell: gcell, floor };
    }
    return { verdict: 'OFF_DOMAIN', reason: `gzip cell ${gcell} ∉ {${authorized.join(', ')}} — scope (agreement=GZIP_ONLY)`, authoritative_cell: gcell, floor };
  }
...
async function main() {
  ...
  const { verdict, reason, authoritative_cell, floor: margin_floor } = decideVerdict({
    result, authorized: args.authorized, agreement, lens: args.lens, ncdMargin, marginConstruct: 'axis',
  });
...
if (isMain) main().catch((e) => { console.error(e.stack || e.message); process.exit(3); });
