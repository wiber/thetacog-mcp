...
// v7-fwmax (2026-09-17): classifyClaim gained an upper bound on function-word ratio (FW_MAX)
// closing the pure-function-word-repetition hole ("the the the …" scored 'meaning' with no
// ceiling) — bumped so every warm cache re-filters through the corrected classifier.
const INGEST_VERSION = 'atomic-claims-v7-fwmax';
...
// FW_MAX — the missing UPPER bound on function-word ratio (2026-09-17 trigger-battery bisect,
// commit c5f13cfcb7's git note has the full trace). classifyClaim's "meaning" gate was
// `fw >= 0.12 && soup < 0.5` with no ceiling, so pure function-word repetition ("the the the
// the the the the the the", fw=1.0, soup=0) cleared it as 'meaning' — the gate was built to
// reject CODE (which reads near fw=0) and never anticipated monotone-repetition noise sitting
// at the OPPOSITE end of the same axis. That single degenerate claim then floods senseDecompose's
// similarity selection downstream (see trigger-battery-degenerate-guard.test.mjs); this is the
// root-cause fix, not the downstream symptom patch.
//
// MEASURED (not picked by taste): ran classifyClaim() over the first 2,000 claimify() segments
// this repo's own ingest sees (docs/ + scripts/gdd/goals + src/ + scripts/ + .thetacog/pmu/src,
// same dirs as ingestIntent/ingestReality, walked read-only — see the measurement script in this
// commit's git note) and recorded the function-word ratio of every claim that already verdicts
// 'meaning' under the fw>=0.12 floor:
//   2,000-claim slice: n=1,625  p50=0.333  p90=0.5   p95=0.529  p99=0.6    max=0.8
//   full corpus (11,226 claimify() segments): n=8,364  p50=0.308  p90=0.5  p95=0.571
//     p99=0.667  p99.9=0.765  max=0.8
// Not one real meaning claim in either population exceeds fw=0.8 (the ceiling holders are short
// real questions like "- ✅ Who is this book for?", fw=0.8, and "- What are we not questioning?",
// fw=0.8 — legitimate prose that happens to be almost entirely function words at low token count).
// FW_MAX=0.85 sits a clean margin above the measured true max (0.8, not just p99) so no observed
// real claim is reclassified, while staying well below 1.0 — the score any claim built ENTIRELY
// of function words (the incident case, and any longer variant: a claim made only of "the of and
// to in on for …" repeated always scores fw=1.0 regardless of how many distinct function words it
// strings together, so a bound below 1.0 catches every such claim, not just the exact repro).
const FW_MAX = 0.85;
...
export function classifyClaim(c) {
  ...
  // not dominated by identifier/class tokens — AND is not itself pure connective
  // tissue with no content word ballast (fw > FW_MAX = monotone function-word noise).
  const meaning = fw >= 0.12 && fw <= FW_MAX && soup < 0.5;
...
// claimSegments — the ONE segmentation both claimify() and claimifyDiagnose() walk. Extracted
// (2026-08-24) so a diagnosis can never describe a different split than the one that actually
// produced the claims: if the two drifted, the diagnosis would be a projection that is not
// sufficient for the question it exists to answer ("why did this corpus yield no claims?").
export function claimSegments(text) {
  ...
  for (const block of String(text ?? '').split(/\n\s*\n/)) {
    ...
    const bySentence = (s) => s.replace(/\s+/g, ' ').split(/(?<=[.!?])\s+(?=[A-Z(`"#-])/);
    const parts = looksCode ? b.split('\n') : bySentence(b);
    for (const part of parts) {
      const c = part.replace(/\s+/g, ' ').trim();
      if (!c) continue;
      // `looksCode` is a TOKEN (`;` `=` `{}`) standing in for the construct "this block is code", and one
      // semicolon in a paragraph of prose flips the whole block to per-line splitting. A caller that
      // flattens a document to one line (encircle-panel.sh) then hands over ONE "line" longer than
      // MAX_CLAIM → tooLong → zero claims → empty reality grid → blank OG panel (205/206 on 2026-09-11).
      // A per-line segment past MAX_CLAIM can never become a claim under either regime, so re-splitting
      // it by sentence loses nothing for code and recovers prose. Narrowed by argument, not by tuning.
      if (looksCode && c.length > MAX_CLAIM) { for (const s of bySentence(c)) { const t = s.trim(); if (t) out.push(t); } continue; }
      out.push(c);
    }
  }
  return out;
...
export function claimify(text) {
  const claims = [];
  for (const c of claimSegments(text)) {
    if (c.length >= MIN_CLAIM && c.length <= MAX_CLAIM && isSemantic(c) && !looksLikeCodeSyntax(c)) claims.push(c);
  }
  return claims;
}

// claimifyDiagnose — WHY a corpus produced the claim count it did, as decidable per-gate counts.
//
// THE INCIDENT (bf-4540, 2026-08-24): commit 3bfedac39 added one line — `// probe` — to one file.
// Its reality corpus is 15 characters, below MIN_CLAIM=20, so ZERO claims is arithmetically
// FORCED: no segment could have survived. panel-repair nevertheless read `0 lit` and escalated it
// to builder as "the real-regression class", the same false-positive shape the 2026-07-22
// docs-only fix closed one rung lower down (that fix taught the classifier about no-CODE commits;
// this one teaches it about no-CLAIM-BEARING-code commits).
//
// The root is a SUFFICIENCY failure, not a sensor failure: `0 lit` alone cannot distinguish
// "the corpus could not possibly have yielded a claim" from "the sensor stopped working", so any
// classifier reading only that number must guess — and guessing wrong costs a builder dispatch.
// This function makes the distinction decidable by reporting which NAMED gate consumed every
// segment. First-gate-wins, so the counts sum exactly to `segments`.
//
// Returns { chars, segments, claimifyKept, kept, rejected: { tooShort, tooLong, notSemantic,
// codeSyntax, soup } } — `claimifyKept` survives claimify's four gates, `soup` is how many of
// those salienceRank's classifyClaim then drops, and `kept` = what actually reaches the lattice.
export function claimifyDiagnose(text) {
  const rejected = { tooShort: 0, tooLong: 0, notSemantic: 0, codeSyntax: 0, soup: 0 };
  const segments = claimSegments(text);
  let claimifyKept = 0;
  for (const c of segments) {
    if (c.length < MIN_CLAIM) { rejected.tooShort++; continue; }
    if (c.length > MAX_CLAIM) { rejected.tooLong++; continue; }
    if (!isSemantic(c)) { rejected.notSemantic++; continue; }
    if (looksLikeCodeSyntax(c)) { rejected.codeSyntax++; continue; }
    claimifyKept++;
    if (classifyClaim(c).verdict !== 'meaning') rejected.soup++;   // salienceRank's gate
  }
  return {
    chars: String(text ?? '').length,
    segments: segments.length,
    claimifyKept,
    kept: claimifyKept - rejected.soup,
    rejected,
  };
}
...
// commentProseBlocks — the same extraction as commentProse, but every kept block carries the
// 1-indexed SOURCE LINE RANGE it came from (C115z, operator: "we should see which parts of what
// files are aperture receipts"). commentProse is unchanged behaviour (out.map(b=>b.text).join
// ('\n\n')) — this is the one place the line map is computed, so a byte offset into the joined
// prose can be walked back to file lines without a second, drifting implementation.
export function commentProseBlocks(text, ext) {
  const out = [];
  const src = String(text || '');
  const push = (lines, startLine, endLine) => {
    const b = lines.join('\n').trim();
    if (b) out.push({ text: b, startLine, endLine });
  };
  if (WIDE_CODE_EXTS.has(ext)) {
    // /* */ blocks first (matches commentProse's own order: block comments, then line comments —
    // NOT overall source order), line numbers from the match's own index into `src`.
    for (const m of src.matchAll(/\/\*[\s\S]*?\*\//g)) {
      const startLine = src.slice(0, m.index).split('\n').length;
      const endLine = startLine + m[0].split('\n').length - 1;
      push(m[0].replace(/^\/\*+|\*+\/$/g, '').split('\n').map((l) => l.replace(/^\s*\*?\s?/, '')), startLine, endLine);
    }
    const lines = src.split('\n');
    let block = [], blockStart = -1;
    for (let i = 0; i < lines.length; i++) {
      const m = lines[i].match(/^\s*\/\/\s?(.*)$/);
      if (m) { if (!block.length) blockStart = i + 1; block.push(m[1]); }
      else if (block.length) { push(block, blockStart, i); block = []; }
    }
    if (block.length) push(block, blockStart, lines.length);
  } else if (WIDE_SHELL_EXTS.has(ext)) {
    const lines = src.split('\n');
    let block = [], blockStart = -1;
    for (let i = 0; i < lines.length; i++) {
      const m = lines[i].match(/^\s*#\s?(.*)$/);
      if (m && !m[1].startsWith('!')) { if (!block.length) blockStart = i + 1; block.push(m[1]); }
      else if (block.length) { push(block, blockStart, i); block = []; }
    }
    if (block.length) push(block, blockStart, lines.length);
  }
...
export function commentProse(text, ext) {
  return commentProseBlocks(text, ext).map((b) => b.text).join('\n\n');
}

// commentProseWithSpans — commentProse's own text (byte-identical: same blocks, same join) PLUS a
// map of each block's BYTE range in that joined text to the SOURCE FILE LINE range it came from
// (C115z). A window picked out of the prose (aperture.rs pick_window's usedStart/usedEnd) is a byte
// range into THIS text, so walking it back to spans[].proseStart/proseEnd tells the reader which
// file lines the chip actually looked at — one map, computed once, never re-derived per caller.
export function commentProseWithSpans(text, ext) {
  const blocks = commentProseBlocks(text, ext);
  let out = '';
  const spans = [];
  for (let i = 0; i < blocks.length; i++) {
    if (i > 0) out += '\n\n';
    const proseStart = Buffer.byteLength(out, 'utf8');
    out += blocks[i].text;
    const proseEnd = Buffer.byteLength(out, 'utf8');
    spans.push({ proseStart, proseEnd, startLine: blocks[i].startLine, endLine: blocks[i].endLine });
  }
