...
const intentText = [specIntent, msg, ...docProse, docsOnly ? '' : diffDocs, ...testSemantic, idealSpec].filter(Boolean).join('\n\n');       // declared intent = (the thread's spec, when realizing) + message + docs + the RULEBOOK (tests + the ideal-case spec on PMU commits)
...
if ((EMAIL || PUBLISH) && !nothingToSay) {
  try {
    ...
    writeFileSync(tmp, body.replace(/(<div style="max-width:700px;margin:0 auto;padding:14px;background:#05070d;color:#c9d1d9">)/, `$1${publicLink}`));
    ...
    if (EMAIL) {
      // ── EMAIL 1 · THE RECEIPT — LLM-FREE, deterministic, sent NOW. This is the insurance artifact;
      // it never waits on and is never blocked by a local model (CLAUDE.md: THE RECEIPT IS LLM-FREE).
      execSync(`node ${resolve(REPO, 'scripts/email-artifact.mjs')} --html ${tmp} --no-attach ${inlineArgs} --attach ${OUT}${tolArg}${audioArg}${reefArg}${bearerArg}${delegArg} --to eliasmoosman@gmail.com --subject ${JSON.stringify(subject)} --from-room`, { cwd: REPO, stdio: 'inherit' });
      // ── EMAIL 2 · THE STORY (the LLM knock-on) — a SEPARATE follow-up subagent call fired AFTER the
      // receipt: the local model writes a rich narrative over the SAME encircled image + full commit
      // message. Detached + non-blocking; a story failure NEVER touches the receipt above. Different type
      // of tool, different email (operator 2026-07-04). NOT gated by FAST — email 2 is already its own
      // detached spawn (never touches email 1's timing), so a fast email 1 must not silently disable it.
      // FAST instead controls email 1's own build: skip the self-test batteries below AND skip the OLDER
      // inline blocking tellStory()/Gemini call above (the actual thing that broke "email 1 is LLM-free").
      try {
        const storyChild = spawn('node', [resolve(REPO, 'scripts/pmu/commit-story.mjs'), '--commit', sha, '--email'],
          { cwd: REPO, detached: true, stdio: 'ignore' });
        storyChild.unref();
        console.log(`   📖 story knock-on spawned (async, LLM, after the receipt) → email 2 for ${shaShort}`);
      } catch (e) { /* the story never blocks the receipt */ }
    } else {
