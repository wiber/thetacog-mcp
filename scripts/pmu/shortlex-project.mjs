...
<p style="font-size:11px;color:#5a6673">Recompute: <code>node scripts/pmu/shortlex-project.mjs --repo</code></p>
    ...
    execSync(`node ${resolve(REPO, 'scripts/email-artifact.mjs')} --html ${tmp} --no-attach ${inlineArgs} --to eliasmoosman@gmail.com --to elias@thetadriven.com --subject ${JSON.stringify(subject)} --from ${JSON.stringify('🧪 Cursor Laboratory · ThetaDriven <laboratory@thetadriven.com>')}`, { cwd: REPO, stdio: 'inherit' });
  }
}
