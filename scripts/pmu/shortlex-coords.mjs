...
// ── THE TOLERANCE BAND — ONE DEFINITION, TWO READERS (the PNG and the ◎ receipt line).
// Chebyshev distance from a block to the fence rectangle; 0 = inside the fence.
export function fenceDist(br, bc, f) {
  if (!f) return 99;
  const dr = br < f.r0 ? f.r0 - br : br > f.r1 ? br - f.r1 : 0;
  const dc = bc < f.c0 ? f.c0 - bc : bc > f.c1 ? bc - f.c1 : 0;
  return Math.max(dr, dc);
}
// distance → the tolerance hue the commit panel paints: in-lane · adjacent bleed · orthogonal drift.
// The text receipt and the encircled PNG MUST read the same band off the same walk, so both call this.
// Previously the PNG banded in three (green/amber/red) while the ◎ line partitioned in two (in/out) from
// its own inline copy of the fence test — the panel and its own caption disagreed by construction.
export function toleranceBand(coord, fence) {
  const { br, bc } = shortLexToBlock(coord);
  if (!Number.isFinite(br) || !Number.isFinite(bc)) return null;
  const d = fenceDist(br, bc, fence);
  return d === 0 ? 'green' : d === 1 ? 'amber' : 'red';
}
