...
export function buildLifecycle(R, links = {}) {
  ...
  const receiptSchema = {
    ...
    // FAIL CLOSED (2026-08-13, Marsh adversarial read): a missing off-lane count must NEVER
    // resolve to a clean pass — for an insurance trigger, absent data is UNMEASURED, not IN_LANE.
    boolean_state: placed === 'UNPLACEABLE' ? 'UNPLACEABLE' : offPct == null ? 'UNMEASURED' : (/^A/.test(String(cell)) && offPct < 25) ? 'IN_LANE' : 'OFF_DOMAIN',
