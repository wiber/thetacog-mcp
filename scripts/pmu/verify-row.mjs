...
//                                    [--targets <file>] [--json] [--report]
// npx thetacog-mcp verify <bundle.json> [--repo <path>] [--text-dir <dir>] [--json] [--report]
// npx thetacog-mcp verify --demo   (verifies the shipped fixture; needs no repo, no --text)
//
// --report (C305b, opt-in, OFF by default): append { ts, commit_sha, verdict } to a LOCAL
// ndjson (scripts/vna/foreign-recompute-log.mjs) — no network call, no user/machine data, and
// it never blocks or changes the printed verdict. This is how a stranger's off-machine verify
// leaves a countable trace at all today; it fires ONLY when the caller types --report.
const VALUED_FLAGS = new Set(['--repo', '--text', '--reef', '--targets', '--text-dir']);
async function maybeReport(reportFlag, commitSha, verdict) {
  if (!reportFlag) return;
  try {
    const { appendForeignRecompute } = await import('../vna/foreign-recompute-log.mjs');
    appendForeignRecompute({ commitSha, verdict });
  } catch {
    /* best-effort only — a verify's verdict is never gated on whether this succeeds */
  }
}
...
export async function runCli(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--report') { flags['--report'] = true; continue; }
  ...
  if (parsed && parsed.schema === BUNDLE_SCHEMA) {
    ...
    if (rust.ok) {
      ...
      await maybeReport(flags['--report'], parsed.commit_sha ?? null, rust.verdict);
    ...
    await maybeReport(flags['--report'], parsed.commit_sha ?? null, result.verdict);
  ...
  await maybeReport(flags['--report'], row && row.sha ? row.sha : null, result.verdict);
  return EXIT_CODE[result.verdict] ?? 2;
}
