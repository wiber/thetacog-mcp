...
//
//   npx thetacog-mcp prove              # the fast chain (~15s): calibration · same-bytes · your-repo
...
// PILLAR 2 · SAME BYTES — the same probe, placed in TWO SEPARATE PROCESSES: byte-identical
...
// ── PILLAR 2 · SAME BYTES (two separate processes, byte-identical) ─────────────
line('▌PILLAR 2 · SAME BYTES — same probe, two processes, byte-identical or fail');
...
line(det ? '  ✓ byte-identical across processes — the verdict is a FACT you can recompute, not a sample'
         : '  ✗ SAME-BYTES CHECK FAILED — report this; nothing below is trustworthy');
...
try {
  ...
  const counts = new Map(); let abstain = 0; const misses = [];
  for (const subj of subjects) {
    ...
    // AN ABSTAIN IS STILL A MEASUREMENT (operator 2026-07-24, the stranger's-first-60-seconds fix):
    // record the nearest tile + how far over the noise floor it landed, so 20/20 abstains render as
    // twenty distance readings — an instrument reporting "your vocabulary vs this library", never a
    // silent shrug a stranger reads as "the tool doesn't work".
    if (bd >= meanSep) { abstain++; misses.push({ bi, bd, over: bd - meanSep }); continue; }
    counts.set(bi, (counts.get(bi) || 0) + 1);
  }
  const tLabel = (i) => { const e = arr[i]; return e && e.row && e.col ? `${e.row},${e.col}` : String(i); };
  ...
  for (const [tile, n] of top) line(`  lane ${tLabel(tile)} ← ${n} commits`);
  if (misses.length) {
    const near = misses.slice().sort((a, b) => a.over - b.over).slice(0, 3);
    line(`  nearest misses (each abstain IS a distance reading, not a silent skip):`);
    for (const m of near) line(`    tile ${tLabel(m.bi)} @ ${m.bd} bits — ${m.over.toFixed(1)} over the noise floor`);
    if (abstain === subjects.length) line('  every commit abstained: your repo speaks a different vocabulary than this library\'s 144 tiles — that is the honest reading. A repo-native reef (bootstrap) lights your own lanes.');
  }
...
line('  · placement verdicts: byte-identical on re-run, ed25519-signed, ~537ps/gate, ~6M walks/s on Apple silicon');
