...
// ── THE SPOKEN READOUT (operator 2026-09-10) ─────────────────────────────────────────────────────
// "vastly shorten the tesseract readout into the chat … you can skim it but it's too long to listen
//  to … two or three, maybe five of the most important red green and amber and circled sections …
//  but we definitely [keep] σ and all the other important measurements, we just don't need to
//  repeat ourselves a million times."
//
// THE REPETITION WAS STRUCTURAL, NOT VERBOSE PROSE. Every region line ended with KIND[k].means — a
// FIXED sentence per colour — so a panel with fourteen regions read the same three sentences
// fourteen times. That is the bulk of the readout and none of it is information after the first
// occurrence. The colour meanings now print ONCE, as a legend, and the region lines carry only what
// differs: coordinate, meaning, block count, and the commit-message slice with its σ.
//
// THE CAP REUSES THE PNG'S OWN RULE rather than inventing a second one (ONE RULE, ONE PLACE): never
// drop a drift region, then fill with the biggest clusters. That rule was already argued for in this
// file — "sparse real panels yield a long tail of 1-block singletons that read as noise" — and the
// long tail is exactly as noisy spoken aloud as it is in a thumbnail. It had simply never been
// applied to the narration, whose own comment said so out loud: "(The HTML view + narration still
// carry every region.)"
//
// NOTHING IS MEASURED AWAY. Every count, every σ, and the full region list still land in the HTML
// render and the JSON; the readout names how many it withheld and where they are, because a silent
// truncation and a short panel print the same thing and they are different claims.
export function sayRegions(regions, { cap = 5 } = {}) {
  const all = Array.isArray(regions) ? regions : [];
  // DRIFT IS NEVER DROPPED, at any cap. A red region is the one thing the panel exists to report,
  // so the cap governs how many EXTRA regions are read, never whether a rupture is mentioned.
  const drift = all.filter((r) => r.kind === 3);
  const rest = all.filter((r) => r.kind !== 3).sort((a, b) => (b.blocks || 0) - (a.blocks || 0));
  const shown = [...drift, ...rest.slice(0, Math.max(0, cap - drift.length))]
    .sort((a, b) => (b.blocks || 0) - (a.blocks || 0));
  return { shown, withheld: all.length - shown.length };
}

// The one-line-per-colour legend, printed once. Only colours actually present are named — a legend
// for an absent colour is three lines of nothing, spoken.
export function sayLegend(regions, KINDS) {
  const present = [...new Set((regions || []).map((r) => r.kind))].sort();
  return present.map((k) => `     ${KINDS[k].name} = ${KINDS[k].means}`);
}
...
// THE ONE HOME (C18, 2026-09-08). Measured on this date: these three triples were re-typed in
// EIGHT places under scripts/ — confidence-overlay, lens-encircled-png, region-narrative,
// project-portfolio, triptych-render, self-lane-panel, and twice more in this file. The spec's C18
// line said "the fifth copy"; it was the eighth. Every one of them paints or CLASSIFIES the same
// panel, so a drift in any one makes the region classifier and the tolerance count disagree — which
// is the exact lie the block below was written to prevent, committed by copy-paste instead of by edit.
export const TOL_HUE = { 1: [30, 145, 80], 2: [255, 176, 0], 3: [255, 59, 59] };   // green · amber · red (EXACT render hues)
// The same three, addressed by name. Call sites that read `const GREEN = [...]` import THIS rather
// than re-typing the triples; the numeric keys stay for the classifier, which is keyed on kind.
export const TOL_RGB = { green: TOL_HUE[1], amber: TOL_HUE[2], red: TOL_HUE[3] };
...
function synthTolerance() {
  const G = TOL_RGB.green, A = TOL_RGB.amber, R = TOL_RGB.red;   // C18: one home, never re-typed
...
async function main() {
  ...
  if (has('--demo')) {
    ...
    // A REFUSAL IS NOT A PANEL. The decoder's all-dark rgba on a below-floor corpus is the honest
    // "UNMEASURED" state; writing it to --png ships it as a reading (the OG image on 205 posts,
    // 2026-09-11). Refuse to emit, name the refusal, exit non-zero — the caller must not get a file.
    if (meta && meta.refused) {
      console.error(`  ✗ REFUSED — ${meta.refusal || 'panel below the lit-mass floor'}; no PNG written, no HTML written.`);
      process.exit(4);
    }
  }
  ...
  for (const r of regions) {
    ...
    // narrative = the FULL line, still written to the HTML and the JSON unchanged.
    ...
    // spoken = the same facts MINUS k.means, which the legend now states once for the whole panel.
    r.spoken = `${k.name} ${shape}${r.coord.label}${r.meaning ? ` · «${r.meaning}»` : ''} (${r.blocks} blk)${r.line ? ` · ${r.line.orient} streak, ${r.line.axis} held` : ''}${slice ? ` · σ${slice.sigma} "${slice.clause.slice(0, 70)}"` : ''}`;
  }
  const tally = [1, 2, 3].map((k) => ({ k, n: regions.filter((r) => r.kind === k).length })).filter((x) => x.n);
  const say = sayRegions(regions, { cap: Math.max(1, parseInt(arg('--say-top', '5'), 10) || 5) });
  console.log(`\n  TOLERANCE PANEL — ${regions.length} colored region${regions.length === 1 ? '' : 's'} encircled (${tally.map((x) => `${x.n} ${KIND[x.k].name}`).join(' · ') || 'none'}):`);
  for (const line of sayLegend(regions, KIND)) console.log(line);
  for (const r of say.shown) console.log(`     ▸ Area ${r.n} · ${r.spoken || r.narrative}`);
  if (say.withheld > 0) console.log(`     … ${say.withheld} smaller region${say.withheld === 1 ? '' : 's'} not read out (every one is in the HTML and the JSON) — --say-top N to hear more`);
