...
function main() {
  ...
  // σ FLOOR — A PRE-CALIBRATION PLACEHOLDER, NOT AN EARNED NUMBER (corrected 2026-07-25).
  // 3.4 was carried as an "earned-gold floor" with no derivation behind it. Measured over the
  // reference corpus (data/pmu/attest-reference-corpus.json): NO document reaches it — the genuine
  // deliverable places at 2.54, and the highest σ in the corpus (2.94) belongs to random tokens,
  // because σ is placement CONFIDENCE, not quality. A floor nothing can clear does not grade work;
  // it stamps REWORK on everything, which is worse than no signal because it looks like a finding.
  // Kept at 3.4 rather than quietly retuned to whatever makes our own demo pass — moving a floor to
  // clear your own showcase is the rubber stamp we are trying not to be. It is labelled honestly
  // instead, everywhere it surfaces, until the held-out study derives a real one.
  const tolSigma = Number(arg('--tolerance-sigma', '3.4'));
  ...
  if (verdict === 'ABSTAIN') {
    ...
    reason = `placement: cell ${cell} at σ=${sigma.toFixed(2)} (floor ${tolSigma}). ${marginal ? `MARGINAL — the work is INSIDE the authorized cells (the decidable part, and it passed); σ is merely below the floor. READ THIS AS UNINFORMATIVE, NOT AS A FINDING: ${tolSigma} is a pre-calibration placeholder no document in the reference corpus reaches, and σ is placement confidence, not quality — random tokens out-score a genuine deliverable on it. The scope verdict above is the decidable result; this flag is advisory noise until the held-out study derives a real floor.` : 'Inside the authorized cells, σ above the floor.'}`;
  }
