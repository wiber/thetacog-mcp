...
//   Oracle  gate           → drives the EXISTING PMU runner
//                            (issue-receipt.mjs --lens gzip → gzip-NCD vs the 12
//                            ShortLex axis nodes → lane argmax) and mints a BOUND receipt
...
// stdin, capture the host-signed gzip-lens receipt it prints.
//
// WHAT ACTUALLY DECIDES THIS VERDICT (corrected 2026-07-25 — the comment here used to say
// "144×144 projection + ballistic walk", which a reviewer can falsify with one grep):
// issue-receipt --lens gzip calls compress() in src/lib/pmu/compress.mjs, which scores the doc by
// gzip-NCD against the 12 ShortLex axis NODES (4 snippets each = 48 anchors) and argmaxes to a LANE.
// The 144-cell pixel placement (compress.mjs placePixel) and the recursive ballistic walk
// (pmu-rust/src/ballistic.rs) are real and run on OTHER paths in this repo — the commit triptych, the
// prompt lens — but neither arbitrates the gate. Do not describe this path as something bigger than
// it is; the whole value of the receipt is that a stranger can check the claim against the code.
// Guarded by tests/pmu-simulator/attest-demo-separation-honesty.test.mjs.
...
async function cmdGate(a) {
  ...
  const body = {
    ...
    oracle: 'thetacog pmu gzip-NCD → 12 ShortLex axis nodes (48 anchors) → lane argmax (compress.mjs)',
  };
