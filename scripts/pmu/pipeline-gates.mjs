...
export function readGate(gate, builtinDefault) {
  try {
    const out = execFileSync('sqlite3', ['-json', DB,
      ...
      // stderr IGNORED: on a fresh install the table doesn't exist yet; sqlite3 prints "no such table"
      // to stderr (a scary-looking RISK INDICATOR in a live demo) even though we catch the throw and fall
      // back to the builtin default. Discard stderr so the fresh-install run is clean.
