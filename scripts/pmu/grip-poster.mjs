...
// NAME THE RULES PULLED UP (operator 2026-09-01: "lets name the rules pulled up"). The 📜 line has
// carried named handles since the pile was stamped; the poster still printed ten anonymous prose
// stubs, so the rules the lens actually reached for could not be referred to, cited in a Sidecar
// note, or checked against the ⚖️ block. Resolution order: the STORED name (reef rule_names /
// lens_rules.name, keyed by text-head ≤60ch — the same key convention every other surface uses),
// else ruleName() — the SAME deterministic namer name-the-pile stamps with, so an unstamped tile
// gets the identical handle it will get when the pile is next stamped. LLM-free either way.
const posterRuleName = (text, ruleNames) =>
  (ruleNames || {})[String(text).slice(0, 60)] || ruleName(text);
...
export function renderGripPoster(coord, { radius = 0, cap = 10, min = 6, promptText = '', heatCoords = null, ruleNames = null } = {}) {
  ...
  const names = top.map((p) => posterRuleName(p.text, ruleNames));
  const lines = top.map((p, i) => `  ${i + 1}. [T${p.slot.tier}·${p.state}] «${names[i]}» ${String(p.text).replace(/\s+/g, ' ').slice(0, 96)}`);
  ...
  return { block, meta: { coord, mode, radius: r, rules: top.length, names, projection_hash, render_ms: render.render_ms, visit_invariant: render.visit_invariant, tiers } };
}
