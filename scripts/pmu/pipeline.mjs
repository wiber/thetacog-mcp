...
import { UNPLACED_SIGMA } from '../../src/lib/pmu/unified-drift.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = resolve(__dirname, '../..');

// THE ONE binary (see src/lib/pmu/pmu-binary.mjs) — same canonical resolution the walk uses, so the
// panel pipeline lands on the same pmu-onchip regardless of calling context. WALK_TIMEOUT_MS is the
// safety valve the panel path was missing: an untimed execSync blocks the whole pipeline FOREVER if
// the child hangs (cache-lock contention — ~8 panel pipelines share vectors.json). A bounded timeout
// turns an infinite freeze into a visible error the empty-heat retry / caller can recover from.
const DAEMON_BIN = () => resolvePmuBinary(REPO_ROOT);
...
function calcSigma(scores) {
  ...
  // ── std === 0 IS NOT ZERO DRIFT, IT IS NO DISCRIMINATION (2026-09-05) ──────────────────────
  // This line returned 0 when the spread across reality scores was flat, and 0 reads downstream as
  // "perfectly in lane". src/lib/pmu/unified-drift.mjs documents that exact defect and calls it an
  // UNDER-ALARM, "EXACTLY backwards": a change whose text shares no vocabulary with any of the 144
  // anchors lights nothing, every score comes back identical, std is 0 — and the panel reported the
  // most reassuring number it has.
  //
  // MEASURED BEFORE CHANGING ANYTHING: across 8,865 graded panels, 62% sit at σ ≤ 0, and among the
  // 2,431 single-file commits — the likeliest off-lattice candidates — it is 65.5%. The honest
  // third verdict shipped on 2026-07-01 as an out-of-band sentinel meaning UNPLACED, "never a
  // spurious 0, never a real z-score". It has never appeared in a single panel, because it lives in
  // walkShape() and THIS pipeline never imports that module. Built, correct, guarded, unwired —
  // the fourth instance of that shape found today.
  //
  // THE SENTINEL IS IMPORTED, NEVER REDEFINED. One rule lives in one place; a second copy of 99
  // here would drift from the first the day either moves, and PMU_UNPLACED_SIGMA already exists to
  // move both together.
  return { top, mean, std, sigma: std > 0 ? (top - mean) / std : UNPLACED_SIGMA, unplaced: !(std > 0) };
}
...
export async function runPipeline(opts = {}) {
  ...
  // opts.axesPath — optional override so a caller (e.g. a /tape session) can walk against its own
  // 144-cell library instead of the repo-wide one. Undefined = loadAxes()'s own default = today's
  // behaviour, unchanged. See pipeline-state.mjs::loadAxes for the anti-clobber Map cache.
  const rawAnchors = loadAxes(opts.axesPath);
...
try {
  ...
  // ── ADMISSIBILITY: NON-EMPTY IS NOT THE SAME QUESTION AS ENOUGH MASS (2026-09-05) ─────────────
  // The check above asks whether the intent corpus exists. It has never asked whether there is
  // enough of it for the reading to mean anything, and the corpus audit measured what that costs:
  // over 8,907 rendered panels, below a ~400-character intent 36.66% read >=90% off-lane, while
  // above it 0.07% do — a ~500x difference in extreme readings produced by how much was typed.
  // chore commits (median 55 chars) read 92%; feat (1,354 chars) reads 19%. Same repo, same walk,
  // same tolerance. The mechanism is two lines away in triptych-render: offPct = orth/total over
  // LIT cells, so a thin intent lights few intent cells, reality has almost nowhere to fire
  // in-lane, and off-lane approaches 100% BY CONSTRUCTION rather than by drift.
  //
  // THE DOOR FOR THIS ALREADY EXISTED AND WAS WIRED TO NOTHING. scripts/pmu/panel-door.mjs states
  // the mechanism exactly — "a text carrying far less than one anchor's worth of mass is being
  // compared against 144 things each heavier than itself, and what comes back is dominated by
  // length" — derives its floor FROM THE SENSOR rather than picking one, and reports UNMEASURED
  // instead of silently "fine". It was imported by five test files and by zero production paths.
  // This wires it. Nothing is invented here; the reading is the door's.
  //
  // IT RECORDS, IT DOES NOT SUPPRESS. The off-lane number still renders exactly as before — a
  // panel that stopped appearing would lose a record, and AXIOM 1 is unambiguous that a discarded
  // record is unpurchasable. What changes is that every panel now CARRIES whether it cleared the
  // floor, so an aggregate can exclude the 3,783 panels that are measuring message length and a
  // reader can tell a measurement from an artifact. Marking is reversible; deletion is not.
  try {
    const { canonicalMassFloor } = await import('./panel-door.mjs');
    const floor = await canonicalMassFloor();
    const gz = (t) => { try { return zlib.gzipSync(Buffer.from(String(t || ''), 'utf8')).length; } catch { return 0; } };
    const intentMass = gz(intentCorpus);
    // floor === null means physics.mjs could not be resolved. That is UNMEASURED, never "clear" —
    // reporting admissible on a floor we failed to read is the false-green this whole check exists
    // to stop, and it would be indistinguishable from a real pass in the panel.
    const admissible = floor == null ? null : intentMass >= floor;
    checks.push({
      name: 'intent corpus clears the mass floor',
      ok: admissible === true,
      detail: floor == null
        ? 'UNMEASURED — canonical floor unresolved (packages/thetacog-mcp/scripts/tape/physics.mjs)'
        : `${intentMass} gzip bytes vs floor ${floor}${admissible ? '' : ' — READING IS LENGTH-DOMINATED, not drift'}`,
    });
    record('admissibility', { intent_gzip_bytes: intentMass, mass_floor: floor, admissible });
  } catch (e) {
    // A door that cannot open reports that it could not open. It never reports a pass.
    checks.push({ name: 'intent corpus clears the mass floor', ok: false, detail: `UNMEASURED — ${e.message}` });
  }
...
let binMethod, binCutoff, binMean, binStd;
  ...
  try {
    ...
    record('sigma', {
      ...
      unplaced: sigmaInfo.unplaced === true,
      band: sigmaInfo.unplaced ? 'unplaced' : (sigmaInfo.sigma >= sigmaFloor ? 'gold' : 'noise')
    });
...
export async function runPartial(from, opts = {}) {
  ...
  const state = loadState();
  const anchors = getSortedAnchors(loadAxes(opts.axesPath));
  const claudbridgeUrl = opts.claudbridgeUrl || process.env.CLAUDBRIDGE_URL || 'http://localhost:7777';
