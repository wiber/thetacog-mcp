...
import { knockOnSection } from './knock-on.mjs';
import { advancedModeSection } from './advanced-mode-section.mjs';
import { resolveLattice, ingestIntegrity } from './lattice-resolve.mjs';
import { autoContextStatus } from './knock-on-auto.mjs';

// INGEST INTEGRITY — the stays-confirmed panel (operator 2026-07-22: "I have to be able to see
// from the page that it's confirmed and it stays confirmed"). Every build independently
// recomputes each rank's available lattice mass and compares it to what the intent corpus
// carried: green only at 100%; red the moment a cap creeps back in. The amber line states the
// honest gap (the auto-tick mining path measures vocab-level, not yet the full dump).
function ingestIntegritySection(resolvedCells, authorized) {
  const ii = ingestIntegrity(resolvedCells, { authorized });
  const rows = ii.rows.map((r) =>
    `<div style="font-size:11px">${r.full} — carried ${r.carried.toLocaleString('en-US')}B of ${r.available.toLocaleString('en-US')}B available · ${r.pct >= 100 ? '<span style="color:#19c37d">100% ✓</span>' : `<span style="color:#d33;font-weight:bold">${r.pct}% TRUNCATED ✗</span>`}</div>`).join('');
  const verdict = ii.ok
    ? `<span style="color:#19c37d;font-weight:bold">FULL DUMP CONFIRMED — ${ii.pct_total}%</span>`
    : `<span style="color:#d33;font-weight:bold">INGEST TRUNCATED — ${ii.pct_total}% ✗ (a cap crept back in)</span>`;
  const auto = autoContextStatus();
  const autoLine = auto.green
    ? `<div style="font-size:10px;color:#19c37d;margin-top:4px">${auto.note}</div>`
    : `<div style="font-size:10px;color:#e0b400;margin-top:4px">${auto.note}</div>`;
  return `<div class="whychip" style="margin:14px 0"><b>INGEST INTEGRITY</b> — recomputed from the lattice files at every build; the box must carry every addressed self-definition in full<br>${verdict}${rows}
${autoLine}
<div style="font-size:10px;color:#e0b400;margin-top:2px">reef-loop miner: vocab-level (separate loop — its full-dump wiring is still queued; this line stays amber until that lands)</div></div>`;
}
...
// ── THE AI GREEKS, READ OFF A SIGNED SIGNAL ──────────────────────────────────────────────────────
// The panel below has been titled "controls & Greeks" since 2026-07-20 while rendering only
// MEASUREMENTS — σ, drift, timings, tier, seal. The five actual Greeks (Δ · Δ-spread · Γ · 𝒱 · Θ,
// src/lib/pmu/greeks.mjs) were NEVER wired into this page: `git log -S computeGreeks` on this file
// returns nothing. Operator, 2026-08-13: "there were measurements just no greeks."
//
// They are not RECOMPUTED here. lens-signal-emit.mjs already writes them into an ed25519-signed
// signal on every commit, so the page reads the newest one — the receipt stays the single source and
// stays LLM-free, and what the page shows is exactly what a stranger can re-verify offline.
// Absent signals → every Greek renders as a dash carrying its own reason, never a fabricated number
// and never a silently-omitted tile (the vanishing-vega failure mode this whole pass exists to fix).
function latestGreeks() {
  for (const dir of ['.thetacog/lens-signals', 'data/pmu/lens-signals']) {
    try {
      const d = resolve(REPO_ROOT, dir);
      const newest = readdirSync(d).filter((f) => f.endsWith('.json'))
        .map((f) => ({ f, mt: statSync(resolve(d, f)).mtimeMs }))
        .sort((a, b) => b.mt - a.mt)[0];
      if (!newest) continue;
      const s = JSON.parse(readFileSync(resolve(d, newest.f), 'utf8'));
      if (s && s.greeks) return { greeks: s.greeks, ref: s.ref || null, id: s.id || null, reference: dir.startsWith('data/') };
    } catch { /* try the next dir */ }
  }
  return null;
}
// The roster, in the order an underwriter reads it: what moved, how far it leaked, whether the rate
// of change is accelerating, how volatile the series is, and what it costs to keep measuring.
const GREEK_ROSTER = [
  ['delta', 'Δ delta', 'weighted Semantic Bleed σ'],
  ['deltaSpread', 'Δ-spread', 'max Chebyshev king-move leak'],
  ['gamma', 'Γ gamma', 'traction slope (drift rate-of-change)'],
  ['vega', '𝒱 vega', 'reef volatility (hold-rate variance)'],
  ['theta', 'Θ theta', 'efficiency decay vs Lens-OFF base'],
];
...
// ── BUILD-TIME CONSTANTS ALLOWLIST (incident 2026-09-17) — the HUD_MAP/paintHud/verifySeal block
// (line ~1900) sits INSIDE buildUX's own outer template literal (the `return \`<!doctype…` that
// closes at the end of this file — see the comment at the TDZ incident, line ~1887), so a bare
// ${CONST} there resolves fine whenever buildUX() actually runs: Node evaluates the enclosing
// backtick and substitutes it, and the real emitted page has always carried the resolved value
// (verified 2026-09-17 by running the pre-fix source through the real CLI: docs/pmu/attest-demo-
// ux.html came out with `const KILL=25;`, never a placeholder — production was never broken).
// What DOES break is the three standalone test harnesses (hud-render-readback, hud-delta-repaint,
// state-projection-equivalence) that slice this block out as raw TEXT via indexOf/slice and
// `new Function(...)` it on its own, with no enclosing backtick left to do the substitution — so
// the bare ${CONST} ships into that extracted text unresolved. resolveBuildConstants/
// extractPaintBlock are the ONE place that repeats what the outer template literal does, so the
// extracted text is safe to evaluate standalone AND a brand-new ${CONST} anywhere in that block
// throws immediately (a build failure) rather than surviving as a silent, un-substituted token.
const PAINT_BLOCK_CONSTANTS = { OFFLANE_KILL_PCT };

export function resolveBuildConstants(chunk, constants = PAINT_BLOCK_CONSTANTS) {
  const resolved = chunk.replace(/\$\{([A-Z0-9_]+)\}/g, (whole, name) => {
    if (!Object.prototype.hasOwnProperty.call(constants, name)) {
      throw new Error(`resolveBuildConstants: unresolved token \${${name}} — not in the allowlist (${Object.keys(constants).join(', ') || 'none'})`);
    }
    return String(constants[name]);
  });
  const stray = resolved.indexOf('${');
  if (stray !== -1) {
    throw new Error(`resolveBuildConstants: a bare \${ survives substitution near: ${JSON.stringify(resolved.slice(stray, stray + 40))}`);
  }
  return resolved;
}

// A self-referential extraction is its own gotcha: THIS function's own source contains the marker
// strings as quoted literals (`src.indexOf('const HUD_MAP = [')` below), so a plain indexOf finds
// ITSELF first, not the real declaration further down the file. Anchor to true line-start (the
// real HUD_MAP/goToState declarations sit at column 0; the quoted literals here sit mid-line,
// preceded by `src.indexOf('`) so the two can never collide.
function findLineStart(src, needle) {
  const re = new RegExp('^' + needle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'm');
  const m = re.exec(src);
  return m ? m.index : -1;
}

// Self-reads THIS file's own source (import.meta.url — never the working tree of a caller) and
// slices out the HUD_MAP → goToState paint block: the one extraction boundary the standalone HUD
// test harnesses need, pre-resolved so it never depends on an enclosing template literal.
export function extractPaintBlock() {
  const src = readFileSync(fileURLToPath(import.meta.url), 'utf8');
  const start = findLineStart(src, 'const HUD_MAP = [');
  const end = findLineStart(src, 'async function goToState');
  if (start < 0 || end < 0 || end <= start) throw new Error('extractPaintBlock: HUD_MAP…goToState markers not found in attest-demo-ux.mjs');
  return resolveBuildConstants(src.slice(start, end));
}
...
export function revertedSection() {
  // AMPUTATED PATHS (underwriter UX trap 2): "show what was un-learned, not just learned."
  // Data source: data/pmu/reverted-receipts.ndjson — emitted BY THE REVERT MECHANISM at revert
  // time (refs + hashes + when, never raw text). Absent file / empty file → renders NOTHING
  // (draws-nothing-until-run; a revert ledger over theoretical data would itself be a forgery).
  let rows = [];
  try {
    rows = readFileSync(resolve(__dirname, '..', '..', 'data/pmu/reverted-receipts.ndjson'), 'utf8')
      .trim().split('\n').filter(Boolean).map((l) => { try { return JSON.parse(l); } catch { return null; } })
      .filter((r) => r && r.ts && (r.what || r.coord)).slice(-6);
  } catch { return ''; }
  if (!rows.length) return '';
  return '<div class="whychip" style="margin:14px 0"><b>AMPUTATED PATHS · what was un-learned</b> — the correction record, as durable as the learning<br>'
    + rows.map((r) => '<div style="margin-top:6px"><b>' + String(r.ts).slice(0, 19) + 'Z</b> · ' + String(r.coord || '') + ' · ' + String(r.what || '').slice(0, 140) + (r.sha ? ' · sha ' + String(r.sha).slice(0, 12) : '') + '</div>').join('') + '</div>';
}

export function throughputSection() {
  // THE MULTIPLES, GRAPHED (operator 2026-07-23: "graph the multiples on the demo page"). Every
  // bar is a MEASURED regime from data/pmu/throughput-multiples.json — never a retyped estimate.
  // The checkbox (log scale) is the only page-JS here and it is a pure CSS-class toggle, no data
  // computed client-side — a bar's width is a data attribute set at generation time.
  let d = null;
  try { d = JSON.parse(readFileSync(resolve(__dirname, '..', '..', 'data/pmu/throughput-multiples.json'), 'utf8')); } catch { return ''; }
  if (!d || !Array.isArray(d.regimes)) return '';
  const maxMult = Math.max(...d.regimes.map((r) => r.multiple));
  const bars = d.regimes.map((r) => {
    const linPct = Math.max(0.4, 100 * r.multiple / maxMult);
    const logPct = Math.max(3, 100 * Math.log10(r.multiple + 1) / Math.log10(maxMult + 1));
    return '<div style="margin:6px 0"><div style="font-size:11px;color:#8892a6">' + esc(r.name) + ' — <b style="color:#66fcf1">' + r.multiple + '×</b> · ' + r.per_sec.toLocaleString() + '/sec</div>'
      + '<div class="tpbar" style="height:14px;background:#1a2333;border-radius:3px;overflow:hidden;margin-top:2px"><i style="display:block;height:100%;background:linear-gradient(90deg,#0a84ff,#66fcf1);border-radius:3px" data-lin="' + linPct.toFixed(1) + '" data-log="' + logPct.toFixed(1) + '" ></i></div></div>';
  }).join('');
  const prov = d.provenance ? '<div style="font-size:11px;color:#66fcf1;margin:4px 0">▸ measured on <b>' + esc(d.provenance.host || '?') + '</b> @ <b>' + esc(d.provenance.commit || '?') + '</b> · ' + esc((d.provenance.measured_at || d.ts || '').slice(0, 19)) + ' · node ' + esc(d.provenance.node || '') + '</div>' : '';
  const seeItYourself = '<div style="font-size:11px;color:#8892a6;margin:4px 0"><b>See it yourself:</b> run <code>node scripts/pmu/measure-throughput.mjs</code> then reopen this page — the bars become YOUR machine\'s numbers, timestamped and re-runnable.</div>';
  const walkFact = d.walk ? '<div style="font-size:11px;color:#8892a6;margin-top:4px"><b>One full walk:</b> ' + esc(d.walk.sensor || '') + ', ' + d.walk.plies + ' plies, ' + d.walk.hops + ' hops over ' + d.walk.footprint + ' coords in ' + d.walk.total_ms + 'ms (' + esc(d.walk.note || '') + ').</div>' : '';
  return '<div class="whychip" style="margin:14px 0"><b>THROUGHPUT MULTIPLES · pre-compute + RAM buys this</b> — every bar MEASURED (data/pmu/throughput-multiples.json), the heavy gzip-NCD math done ONCE offline; the live path is an in-RAM O(1) lookup (a speed class, not a hardware-cache claim)'
    + prov + seeItYourself
    + '<label style="display:block;font-size:11px;color:#8892a6;margin:6px 0"><input type="checkbox" id="tpLog" checked onchange="document.querySelectorAll(\'.tpbar i\').forEach(function(b){b.style.width=(this.checked?b.dataset.log:b.dataset.lin)+\'%\'}.bind(this))"> log scale (the multiples span orders of magnitude)</label>'
    + bars
    + walkFact
    + '<div style="font-size:11px;color:#8892a6;margin-top:6px"><b>Precision preserved:</b> ' + esc(d.custody) + '</div>'
    // SPLIT CLOSE TAG, ALWAYS: a literal '</scr'+'ipt>' anywhere in this module is a page-killer waiting
    // for someone to move it inside the main client template — the browser ends the script at the first
    // literal close, wherever it appears. Emitting it split makes that impossible by construction and
    // keeps the source-grep guard (attest-demo-ux-airgapped #2) honest instead of permanently red.
    + '<script>document.querySelectorAll(\'.tpbar i\').forEach(function(b){b.style.width=b.dataset.log+\'%\'});<' + '/script>'
    + '</div>';
}

export function contextCaliperSection() {
  // THE REPO CALIPER, ON THE PAGE (operator 2026-07-23: "how can we tell from the demo page that
  // the optimal context is included?"). Answer: you can tell because the page shows BOTH sides of
  // the trade and names which side is unmeasured. A panel that only showed the cost would be the
  // INGEST INTEGRITY mistake again — a green light on a number nobody proved was the right one.
  let c = null;
  try { c = JSON.parse(readFileSync(resolve(__dirname, '..', '..', 'data/pmu/context-caliper.json'), 'utf8')); } catch { return ''; }
  if (!c || !Array.isArray(c.rows)) return '';
  const rows = c.rows.map((r) => `<tr><td style="padding:2px 10px 2px 0">${r.cap === 0 ? 'none' : r.cap + 'ch'}</td><td style="padding:2px 10px 2px 0">${r.mode}</td><td style="padding:2px 10px 2px 0">${r.accuracy_pct}%</td><td style="padding:2px 10px 2px 0">${r.delta_vs_baseline >= 0 ? '+' : ''}${r.delta_vs_baseline}pp</td><td style="padding:2px 0">+${r.mean_added_bytes}B</td></tr>`).join('');
  const flag = c.saturated
    ? '<div style="border-left:3px solid #e0b400;padding-left:10px;margin-top:8px"><b style="color:#e0b400">BATTERY SATURATED — this panel cannot answer its own question yet.</b> The ungrounded baseline already routes ' + c.labeled_prompts + '/' + c.labeled_prompts + ', so every grounding level reads identical BY CONSTRUCTION. What grounding COSTS is measured (separation falls monotonically with cap). What it BUYS is <b>not measured</b>, and no number here should be read as an optimum.</div>'
    : '<div style="border-left:3px solid #19c37d;padding-left:10px;margin-top:8px"><b style="color:#19c37d">measured optimum: ' + c.optimum.cap + 'ch (' + c.optimum.mode + ')</b> — ' + c.optimum.accuracy_pct + '% routing accuracy, +' + c.optimum.mean_added_bytes + 'B mean. Live cap is ' + c.live_cap + 'ch.</div>';
  return '<div class="whychip" style="margin:14px 0"><b>THE REPO CALIPER · what context COSTS vs what it BUYS</b> — the intent box is grounded with axis descriptors; this is the both-sided measurement of that choice, recomputed by <code>node scripts/pmu/context-caliper.mjs</code>'
    + '<table style="margin-top:8px;font-size:11.5px"><tr style="color:#8892a6"><td style="padding-right:10px">cap</td><td style="padding-right:10px">packing</td><td style="padding-right:10px">routing acc.</td><td style="padding-right:10px">Δ baseline</td><td>added bytes</td></tr>' + rows + '</table>'
    + flag
    + '<div style="margin-top:8px;font-size:11px;color:#8892a6"><b>Limit:</b> ' + String(c.honest_limit || '') + '</div>'
    + (c.what_would_measure_it ? '<div style="margin-top:4px;font-size:11px;color:#8892a6"><b>What would measure it:</b> ' + c.what_would_measure_it + '</div>' : '')
    + '</div>';
}

export function milestonesSection() {
  // THE LINE, NOT THE DOT (operator 2026-07-22): an underwriter prices trajectory. Every
  // milestone below is a RECEIPT — a file on disk with a measured number — or a dated ruling
  // in the spec. Pure static HTML built at generation time: zero page JS, zero TDZ surface.
  const R = (rel) => { try { return JSON.parse(readFileSync(resolve(__dirname, '..', '..', rel), 'utf8')); } catch { return null; } };
  const mi = R('data/pmu/mass-index-receipt.json');
  const pkg = R('package.json');
  const tape = R('docs/pmu/attest-flight-tape.json');
  const rows = [];
  if (mi) {
    rows.push('<b>Verbatim chain of custody</b> — ' + mi.chunks.toLocaleString() + ' chunks indexed in ' + mi.secs + 's; NCD measures the stripped text, the receipt records raw_sha + raw_len of the ORIGINAL bytes (guard: mass-index-custody, hash-match on resolve). The tape points at real bytes, never our abstraction.');
    rows.push('<b>Self-governance, on the record</b> — measured dedupe ' + mi.dropped_pct + '% vs the stated ' + mi.dedupe_bar_pct + '% bar → rolling-hash near-dup <b>' + mi.rolling_hash_verdict + '</b>. The engine measured its own physics and declined a feature by arithmetic, not opinion.');
  }
  const tsPin = pkg && pkg.devDependencies && pkg.devDependencies['tree-sitter'];
  if (tsPin) rows.push('<b>Resolution horizon, already pinned</b> — tree-sitter ' + tsPin + ' (exact) behind an engine interface: AST snapping when present, brace-heuristic ALWAYS (fallback guard-tested, skip-if-absent). Same math, finer pixels; a native build failure cannot touch the running index.');
  if (tape && Array.isArray(tape.timeline_events) && tape.timeline_events.length) {
    const ids = tape.timeline_events.map((e) => String(e.id || ''));
    const newest = ids[ids.length - 1];
    rows.push('<b>The immutable tape</b> — ' + tape.timeline_events.length + ' live events (newest ' + newest + '), one write door (mkdir lock, fresh-read inside, monotonic ids), overflow auto-archived append-only. The chain of custody for every seat/refusal decision.');
  }
  rows.push('<b>Persistence discipline</b> — SQLite migration FROZEN until post-demo (spec ruling 2026-07-22): one lock discipline in production at a time. The unfreeze will be its own justified, guarded commit.');
  if (!rows.length) return '';
  const stamp = mi && mi.ts ? ' · <span style="color:#66fcf1">receipt stamped ' + String(mi.ts).replace('T', ' ').slice(0, 19) + 'Z — restamped on every index rebuild; the live tape counter above is the pulse between stamps</span>' : '';
  return '<div class="whychip" style="margin:14px 0"><b>MILESTONES · the line, not the dot</b> — each entry is a receipt on disk, recompute it yourself' + stamp + '<br>' + rows.map((r) => '<div style="margin-top:6px">' + r + '</div>').join('') + '</div>';
}
...
export function buildUX(R, extra = {}) {
  ...
  // THE LATTICE RESOLVER (2026-07-22, operator: the intent box read 258 standalone bytes and
  // never resolved its addresses — the NCD measured against an empty room). The intent corpus
  // now resolves the authorized cells + any explicit addresses through the lattice: each
  // addressed cell contributes its title + full corpus, and the custody note names exactly
  // which cells collided. The box text stays the scenario sentence; the MEASURED corpus is the
  // resolved one, and the provenance panel shows the real mass.
  const intentResolved = resolveLattice(DEFAULT_SCENARIO.intent, { authorized });
  const intentText = intentResolved.corpus;
  ...
  const ingestProv = {
    intent: { ...prov('intent_corpus.txt', intentText),
      note: 'LATTICE-RESOLVED — collided cells: ' + intentResolved.cells.map((c) => `${c.full} (${c.bytes}B)`).join(' · ') + ` · raw box ${intentResolved.raw_bytes}B → resolved ${intentResolved.resolved_bytes}B`,
      resolved_cells: intentResolved.cells },
  ...
  // RENDER-TIME PAINT for the last two standing dashes (operator 2026-07-21: 'we should never see
  // empty values — once and for all'). The build KNOWS the newest sealed drift and can TIME a real
  // sha256 of the newest seal right here — so the page ships with values, and the browser's live
  // painters (HUD_MAP for drift, verifySeal for hash) OVERWRITE them the moment they run. Same
  // single-projection law: one live painter each; the build value is the boot frame, never a rival.
  let bootDrift = null, bootHashMs = null, bootCheb = null, bootChebD = null;
  try {
    const withDrift = embeddedTape.filter((e) => e && e.metrics && e.metrics.drift != null);
    if (withDrift.length) bootDrift = withDrift[withDrift.length - 1].metrics.drift;
    // the Greeks describe the newest WALK — administrative states (SPLIT verdicts) carry none.
    // The band tile's σ falls back to the newest walk-carrying tape state when the run receipt
    // is empty (the recDoc-empty class: fourth tile, same disease as unsigned/drift/hash-time).
    const withWalk = embeddedTape.filter((e) => e && e.metrics && e.metrics.walk && e.metrics.walk.trajectory_drift_sigma != null);
    if (withWalk.length) {
      const w = withWalk[withWalk.length - 1].metrics.walk;
      bootCheb = +w.trajectory_drift_sigma;
      if (w.continuous_chebyshev_delta != null) bootChebD = (+w.continuous_chebyshev_delta >= 0 ? '+' : '') + (+w.continuous_chebyshev_delta).toFixed(3);
    }
    const sealedStates = embeddedTape.filter((e) => e && e.seal);
    if (sealedStates.length) {
      const t0h = performance.now();
      createHash('sha256').update(JSON.stringify(sealedStates[sealedStates.length - 1].inputs || {})).digest('hex');
      bootHashMs = Math.max(1, Math.round(performance.now() - t0h));
      // NEVER-UNSIGNED (operator 2026-07-21): the sig tile's LIVE painter reads tape states, but
      // its BOOT read the run's receipt doc — two sources, and the doc is unsigned at build while
      // the tape's newest seal is ed25519-signed. Boot from the SAME source the live painter owns
      // (single-projection law applied to provenance): newest sealed tape state, which the browser
      // then repaints per selected state. 'unsigned' may now only ever mean 'the tape has no seal'.
      const lastSeal = sealedStates[sealedStates.length - 1].seal;
      if (!sealed.signed && lastSeal && lastSeal.signed) {
        sealed.signed = true;
        if (!sealed.receiptId && lastSeal.content_sha256) sealed.receiptId = String(lastSeal.content_sha256).slice(0, 10);
      }
    }
  } catch { /* boot values stay null — tiles fall back to the labeled dash */ }
  ...
  let subWellCount = 0;
  try { subWellCount = readdirSync(resolve(REPO_ROOT, 'data/pmu/reef-l1')).filter((f) => f.endsWith('.json') && !f.endsWith('.nogo.json') && !f.endsWith('.building.json')).length; } catch { subWellCount = 0; }
  ...
  // TESSERACT PROGRESS — the full-structure realization ledger (scripts/pmu/tesseract-progress.mjs
  // writes it; the loop's ticks move it; delta_since_last is the offline counter). Read at build,
  // never restated — same contract as the ratchet card.
  let tessProgress = null;
  try { tessProgress = JSON.parse(readFileSync(resolve(REPO_ROOT, 'data/pmu/tesseract-progress.json'), 'utf8')); } catch { /* run tesseract-progress.mjs */ }
  // meaning history series for the sparkline (same idiom as the ratchet card — build-time SVG)
  let meaningHist = [];
  try {
    meaningHist = readFileSync(resolve(REPO_ROOT, 'data/pmu/meaning-history.ndjson'), 'utf8')
      .trim().split('\n').map((l) => JSON.parse(l)).slice(-60);
  } catch { /* accruing */ }
  // 12×12 meaning heat-map — per-cell carried contribution, red-ringed when nearest-NCD < λ
  function meaningHeatSvg(m) {
    if (!m || !Array.isArray(m.per_cell) || m.per_cell.length === 0) return '';
    const pc = m.per_cell.slice(0, 144);
    const max = Math.max(...pc.map((c) => c.kb), 0.01);
    const lam = (m.duplication && m.duplication.lambda) || 0.24;
    const S = 13, P = 2;
    const rects = pc.map((c, i) => {
      const x = P + (i % 12) * S, y = P + Math.floor(i / 12) * S;
      const op = Math.max(0.06, Math.min(1, c.kb / max));
      const dup = c.nn < lam;
      return '<rect x="' + x + '" y="' + y + '" width="' + (S - 1) + '" height="' + (S - 1) + '" fill="var(--cy)" opacity="' + op.toFixed(2) + '"' +
        (dup ? ' stroke="#ff5d52" stroke-width="1.2"' : '') + '><title>' + esc(c.coord) + ' — ' + c.kb + 'kb carried · nn ' + c.nn + ' · local S/N ' + (c.snr != null ? c.snr : '—') + (dup ? ' — SATURATED (split co-signal armed)' : '') + '</title></rect>';
    }).join('');
    return '<svg viewBox="0 0 ' + (12 * S + 2 * P) + ' ' + (12 * S + 2 * P) + '" width="160" height="160" role="img" aria-label="per-cell carried meaning">' + rects + '</svg>';
  }
  function meaningSparkSvg(rows) {
    if (!rows || rows.length < 2) return '<span class="dim" style="font-size:11px">trend accruing — ' + (rows ? rows.length : 0) + ' point(s); every mass-moving cycle adds one</span>';
    const vals = rows.map((r) => r.carried_meaning_kb);
    const min = Math.min(...vals), max = Math.max(...vals), span = Math.max(0.001, max - min);
    const W = 220, H = 44;
    const pts = vals.map((v, i) => (i * (W - 4) / (vals.length - 1) + 2).toFixed(1) + ',' + (H - 4 - (v - min) * (H - 8) / span + 2).toFixed(1)).join(' ');
    return '<svg width="' + W + '" height="' + H + '" style="vertical-align:middle"><polyline points="' + pts + '" fill="none" stroke="var(--cy)" stroke-width="1.5"/><title>carried meaning kb over the last ' + vals.length + ' mass-moving cycles (' + min + ' → ' + max + 'kb)</title></svg>';
  }
  ...
  // THE SELF-DEFINERS, IN FULL (operator 2026-07-22: "the full semantic payloads from self
  // definers are not clarified on the demo page anywhere — they should be"): the L1 sub-wells'
  // child dumps join the embed so the inspector can show EVERY payload the sensor owns —
  // depth-0's 144 + every materialized child — verbatim, nothing truncated.
  const wellsL1 = {};
  try {
    const l1dir = resolve(REPO_ROOT, 'data/pmu/reef-l1');
    for (const f of readdirSync(l1dir).filter((x) => x.endsWith('.json') && !x.includes('.nogo') && !x.includes('.building'))) {
      const w = JSON.parse(readFileSync(resolve(l1dir, f), 'utf8'));
      wellsL1[String(w.parent)] = (w.cells || []).filter((c) => String(c.snippet || '').length >= 40)
        .map((c) => ({ coord: String(c.coord), snippet: String(c.snippet || '') }));
    }
  } catch { /* no wells — inspector shows depth-0 only */ }

  // Everything the client needs, embedded. No network, no model — the browser recomputes with gzip.
  const DATA = {
    sealed, authorized, cells144, wellsL1, lifecycleHref, axisMap, calib, calibHistory, convergence, banks, dim2mass,
  ...
  // SEMANTIC VEGA is the differentiator and it must never silently disappear. It vanished on every
  // published install because the ledger it reads was stripped by the files allowlist AND this tile
  // was written `vega != null ? tile : ''` — a missing number rendered as a missing concept.
  const vegaWhy = vega != null ? null
    : (SERIES_SNAPSHOT ? 'series too short — needs ≥8 graded commits' : 'no measure-history ledger on this install yet');
  const G = latestGreeks();
  const greekTiles = GREEK_ROSTER.map(([key, sym, what]) => {
    const g = (G && G.greeks && G.greeks[key]) || null;
    const has = g && g.value != null;
    const why = g && !has ? (g.note || 'unavailable') : (G ? '' : 'no signed lens signal on this install yet');
    const shown = has ? (typeof g.value === 'number' ? String(+Number(g.value).toFixed(6)) : String(g.value)) : '—';
    return `<div class="tile" title="${esc(has ? (g.note || what) : why)}"><div class="tv" style="color:${has ? 'var(--cy)' : 'var(--dim)'};font-size:${shown.length > 7 ? '15px' : '19px'}">${esc(shown)}</div><div class="tk">${sym} — ${esc(what)}</div></div>`;
  }).join('\n    ');
  ...
  const chebSigma = sealed.sigma != null ? sealed.sigma : bootCheb;   // run receipt → newest tape walk → dash
  const chebLo = chebSigma != null ? Math.max(0, chebSigma - 2 * 1).toFixed(2) : '—';
  const chebHi = chebSigma != null ? (chebSigma + 2 * 1).toFixed(2) : '—';
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .whychip{flex:1 1 210px;background:#0a0e17;border:1px solid var(--line);border-radius:6px;padding:10px 12px;font-size:12px;line-height:1.5;overflow-x:auto;min-width:0}
  ...
  .tsbars{overflow-x:auto;white-space:nowrap;padding-bottom:4px}
  /* administrative states (SPLIT verdicts) own no panels — an img with no src renders NOTHING, never a broken icon (the broken-modal incident 2026-07-21) */
  .hudenc img:not([src]){display:none}
  ...
  <div class="sub" style="border-left:3px solid #66fcf1;padding-left:10px;margin-top:8px"><b>The concession, before any claim:</b> <b>WHERE</b> an action landed is decidable — provable, re-runnable, no model in the verdict. <b>WHETHER the work is good is undecidable (Rice)</b> — this instrument never claims that, and never will. Every number on this page carries the command that recomputes it; you are not asked to trust anything you cannot re-run.</div>
  ...
  <div class="floathud" id="floatHUD">
    ...
    <div id="hudBody">
      ...
      <div class="hudenclbl" id="encSrcNote" style="text-align:center;color:#f0b429"></div>
  ...
  <div class="greeksgrid">
    ...
    <div class="tile" title="${esc(vega != null ? `population σ of the drift series over the last ${SERIES_SNAPSHOT.n} graded commits` : vegaWhy)}"><div class="tv" style="color:${vega != null ? 'var(--cy)' : 'var(--dim)'}">${esc(vega != null ? String(vega) : '—')}</div><div class="tk">semantic vega — σ volatility (lens)</div></div>
    <div class="tile" title="paints LIVE in the browser: this gauge is owned by the metal off_pct in the live render (single-projection law — one painter, guarded); the build-time dash is by design, and the value appears the moment the local render runs"><div class="tv" id="g-drift" style="color:var(--am)">${bootDrift != null ? Math.round(bootDrift) + '%' : '—'}</div><div class="tk">drift toward excluded</div></div>
    ...
    <div class="tile"><div class="tv" style="font-size:15px" id="g-hashtime">${bootHashMs != null ? '≈' + bootHashMs + 'ms' : '—'}</div><div class="tk">local hash time</div></div>
  ...
  <h3 style="margin:18px 0 8px;font-size:14px">The Greeks — priced off the drift series, not the price series</h3>
  <div class="greeksgrid">
    ${greekTiles}
  </div>
  <p class="legend"><b>Everything above this line is a measurement. These five are the Greeks.</b> An option desk takes the volatility of a <i>price</i> series; there is no second construction that takes it of a <b>semantic</b> one — that is what <b>𝒱 vega</b> is here, and it is the reason a drift series can be underwritten at all. Each value is read from the newest <b>ed25519-signed lens signal</b>${G && G.ref ? ` (<code>${esc(G.ref)}</code>${G.reference ? ', shipped reference' : ''})` : ''}, never recomputed for display and never produced by a model — <code>npx thetacog-mcp attest</code> re-derives the same numbers offline. A dash is an honest absence with its reason in the tooltip (hover it): a Greek whose source has no data <b>stays null</b> rather than becoming a plausible number, which is the only version of this instrument an underwriter can price.${G ? '' : ' <b>Dashes on a fresh install are correct</b> — no reference numbers are shipped, because these Greeks are supposed to be measured on <i>your</i> repository. Run <code>npx thetacog-install-hooks</code> and commit once: the post-commit hook emits your first signed signal and this row fills with your own.'}</p>
    </div>
  </div>
  ...
  <div class="card">
    <div style="display:flex;gap:18px;flex-wrap:wrap;align-items:center;margin-bottom:10px">
      ...
      <button class="rawbtn" id="loadTickTriad" style="border-color:var(--cy);color:var(--cy)" title="THE UNIFICATION: fill ①②③ from the newest harvest seal's own Intent/Reality/Negative — the tick loop and this demo are ONE pipeline">⟲ load latest tick triad</button><span class="dim" id="tickTriadInfo"></span>
      ...
      <label class="chk" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;cursor:pointer" title="the 0.5s tape/viewport/version poll plus the 5s loop poll — off saves battery; the page then only updates when you act"><input type="checkbox" id="livePoll" checked> live polling — auto-update this page (uses battery)</label>
  ...
  <!-- 🧊 TESSERACT PROGRESS (2026-07-20 — operator: "we need a progress bar for how far the ticks
       took the repo, and if they are useful"). Level-by-level realization of the FULL tesseract
       (not the shadow's repo-grip): quantum = chars of placed cell text. Read from
       data/pmu/tesseract-progress.json at build; delta_since_last is the offline counter. -->
  ${tessProgress ? `<h2>Tesseract progress — how far the ticks took the repo <span class="dim" style="font-size:12px">(quantum = chars of placed cell text · recompute: node scripts/pmu/tesseract-progress.mjs)</span></h2>
  <div class="card">
    ${[
      { label: 'L0 shadow — 144 melded cells at the gate floor', num: tessProgress.l0.at_or_above_gate_floor, den: 144, extra: `${(tessProgress.l0.chars / 1000).toFixed(1)}k ch` },
      { label: 'L1 sub-wells materialized (a cell splits under pressure)', num: tessProgress.l1.subwells_materialized, den: 144, extra: `${tessProgress.l1.subwells.map((s) => s.cell).join(' ') || '—'} · ${(tessProgress.l1.chars / 1000).toFixed(1)}k ch` },
      { label: 'depth-1 positions realized (144^(d+1) descent)', num: tessProgress.descent.realized_positions, den: tessProgress.descent.depth1_potential, extra: `${tessProgress.descent.realized_pct_of_depth1}%` },
      { label: 'shadow grip on the repo (218 sections, gzip-NCD)', num: Math.round((tessProgress.shadow_grip_pct || 0) * 10), den: 1000, extra: `${tessProgress.shadow_grip_pct}%` },
    ].map((b) => {
      const pct = Math.max(0.4, Math.min(100, 100 * b.num / Math.max(1, b.den)));
      return `<div style="margin:8px 0"><div style="display:flex;justify-content:space-between;font-size:12px"><span>${esc(b.label)}</span><span class="dim">${esc(String(b.extra))}</span></div>`
        + `<div style="height:8px;background:rgba(120,140,160,.15);border-radius:4px;overflow:hidden"><div style="height:100%;width:${pct}%;background:var(--cy)"></div></div></div>`;
    }).join('')}
    <div style="display:flex;gap:18px;flex-wrap:wrap;align-items:center;margin-top:10px">
      <div>${meaningHeatSvg(tessProgress.meaning)}<div class="dim" style="font-size:10px;text-align:center">per-cell carried meaning · red ring = near-copy under λ${tessProgress.meaning && tessProgress.meaning.duplication ? ' (' + tessProgress.meaning.duplication.pct + '% dup)' : ''}</div></div>
      <div><div style="font-weight:700;font-size:12px">carried meaning <span class="dim" style="font-weight:400">(non-redundant compressed kb)</span></div>${meaningSparkSvg(meaningHist)}
      <div class="dim" style="font-size:11px">${tessProgress.meaning && tessProgress.meaning.trend && tessProgress.meaning.trend.kb_per_hour_24h != null ? tessProgress.meaning.trend.kb_per_hour_24h + ' kb/h (24h) · marginal ' + (tessProgress.meaning.trend.marginal_kb_per_seal != null ? tessProgress.meaning.trend.marginal_kb_per_seal + ' kb/seal' : '—') : 'velocity of meaning: accruing (' + ((tessProgress.meaning && tessProgress.meaning.trend && tessProgress.meaning.trend.points) || 0) + ' points)'}</div></div>
    </div>
    <p class="legend" style="margin-top:10px"><b>THE TICK LEDGER (the denominator):</b> ${tessProgress.ticks_total ? `${tessProgress.ticks_total.toLocaleString()} ticks logged · lifetime seal hit-rate <b>${tessProgress.per_tick.seal_hit_rate_pct}%</b> (${tessProgress.sealed_states}/${tessProgress.ticks_total})${tessProgress.delta_since_last && tessProgress.delta_since_last.ticks ? ` · since baseline: <b>${tessProgress.delta_since_last.ticks} ticks → +${tessProgress.delta_since_last.sealed_states} seals · +${(tessProgress.delta_since_last.carried_meaning_kb ?? 0)}kb carried</b>` : ''} — design ceiling ~4 stmts per mass-moving tick; the gap to actual is the efficiency number.` : 'no loop log on this checkout.'} Breadcrumbs — the more-than-two-dimensions witness: <b>${tessProgress.breadcrumbs.walks.toLocaleString()}</b> recorded walks · <b>${tessProgress.breadcrumbs.hops.toLocaleString()}</b> hop-cells, each carrying <b>actor ⊕ patient ⊕ ply ⊕ grip</b> (max ply ${tessProgress.breadcrumbs.max_ply}) · <b>${tessProgress.sealed_states}</b> sealed states on the tape.${tessProgress.velocity ? ` <b>Velocity:</b> ${tessProgress.velocity.h24.per_hour} stmts/h over 24h (+${tessProgress.velocity.h24.derived} across ${tessProgress.velocity.h24.seals} seals) · 1h ${tessProgress.velocity.h1.per_hour}/h · <b>${tessProgress.child_statements ?? 0}</b> child statements stocked (seals-into-depth).` : ''}${tessProgress.meaning ? ` <b>S/N ${tessProgress.meaning.snr ?? '—'}</b> — signal ${tessProgress.meaning.carried_meaning_kb}kb / noise ${tessProgress.meaning.noise_kb}kb of ${tessProgress.meaning.mass.compressed_kb}kb compressed mass (${tessProgress.meaning.mass.raw_kb}kb raw). <b>Carried meaning:</b> ${tessProgress.meaning.carried_meaning_kb}kb non-redundant compressed (Σ gz·nearestNCD, ${tessProgress.meaning.cells_measured} cells) · density ${tessProgress.meaning.density} · orthogonality ${tessProgress.meaning.orthogonality} · role-coherence ${tessProgress.meaning.role_coherence} — structure measured, quality never claimed (Rice).` : ''} Next split, computed not committed: <b>${esc(tessProgress.next_split.candidate)}</b> — ${esc(tessProgress.next_split.why.split('.')[0])}. ${tessProgress.delta_since_last ? `<b>Since ${esc(tessProgress.delta_since_last.vs_head)}:</b> +${(tessProgress.delta_since_last.l0_chars / 1000).toFixed(1)}k L0 ch · +${tessProgress.delta_since_last.sealed_states} seals · +${tessProgress.delta_since_last.walks} walks — the ticks' yield, counted.` : 'This snapshot is the baseline — the next run counts what the ticks generated while you were away.'}</p>
  </div>` : ''}
  ...
  <div class="card">
    ...
    <details class="reefinspect" id="reefInspect"><summary>🪸 <b>The self-definers, in full</b> — <span id="reefValidation">validating…</span> <span class="dim">(these ARE the ground truth: every verdict compresses against exactly these bytes — click any cell for its FULL payload, and cells with a materialized sub-well show every child dump beneath, verbatim, nothing truncated)</span></summary>
...
function renderReefInspector(){
  ...
  if(!host.__wired){ host.__wired = true; host.addEventListener('click', function(e){
    ...
    var base = cell
      ? full+' — '+String(cell.snippet||'').length+' ch (the self-definer payload, verbatim)'+NL2+String(cell.snippet||'')
      : full+' — MISSING from the embedded reef (the build gate should have refused this page)';
    var wl = (typeof D !== 'undefined' && D && D.wellsL1 && D.wellsL1[coord]) || null;
    if(wl && wl.length){
      base += NL2+'━━ MATERIALIZED SUB-WELL — '+wl.length+' child self-definers ('+Math.round(wl.reduce(function(a,c){return a+c.snippet.length;},0)/1000)+'k ch), each in full ━━';
      for(var wi=0; wi<wl.length; wi++){ base += NL2+'▸ '+coord+'/'+wl[wi].coord+' — '+wl[wi].snippet.length+' ch'+NL2+wl[wi].snippet; }
    }
...
function setBool(state){
  // ONE VERDICT PAINTER (2026-07-21, the split-brain modal: title IN_LANE, banner Out-of-lane 36%,
  // scrubber OFF_DOMAIN — the placement-only path set the banner but never the floating title).
  // setBool now owns EVERY verdict text on the page: boolState AND the floating hudState. A new
  // verdict surface joins THIS function or it will drift — the single-projection law, verdict half.
  const c = state==='IN_LANE' ? '#46d369' : state==='OFF_DOMAIN' ? '#ff5d52' : '#f0b429';
  const el = $('boolState');
  if(el){ el.textContent = state; el.style.color = c; el.parentElement.style.borderColor = c; }
  const hs = $('hudState');
  if(hs){ hs.textContent = state; hs.style.color = c; }
}
...
function paintVerdictFromMetal(V){
  ...
  // OFF_DOMAIN is JUSTIFIED by either failure mode: A (drift past the limit) or B (closer to the
  // excluded domain). Only when NEITHER holds is the record incoherent. The previous paintsInLane
  // clause flagged every older record lacking a MODE tag even when its drift plainly justified
  // the verdict (the 45.8%-vs-35% false alarm, 2026-07-22) — a checker crying wolf teaches the
  // operator to ignore the one real alarm it exists for.
  const bJustified = (V.dN!=null && V.dI!=null && V.dN < V.dI);
  const aJustified = (drift!=null && threshold!=null && drift > threshold);
  const incoherent =
       (tag==='IN_LANE' && (drift > 50 || drift > threshold))
    || (tag==='OFF_DOMAIN' && !aJustified && !bJustified)
...
// (HUD_MAP hoisted above the boot paint — the TDZ incident 2026-07-21: the boot try{goToState} ran 39 lines before the const and killed ALL page init)
...
const HUD_MAP = [
  ...
  { id: 'g-offlane',   get: (st) => { const w=st.metrics&&st.metrics.walk; return w&&w.off_pct!=null ? (w.off_pct+'%') : null; },
                       color: (st) => { const w=st.metrics&&st.metrics.walk; const KILL=${OFFLANE_KILL_PCT}; // fixed FINPRO trigger, never re-derived from the drift dial — ONE source of truth (tile-verdict-colour-live.test.mjs); resolved here by buildUX's own outer template literal, and by resolveBuildConstants()/extractPaintBlock() (line ~100) for any standalone extraction (hud-render-readback.test.mjs etc.)
                         if(!w||w.off_pct==null) return 'var(--dim)';
                         return (+w.off_pct >= KILL) ? 'var(--rd)' : 'var(--gn)'; } },
  { id: 'g-cheb',      keep: true, get: (st) => { const w=st.metrics&&st.metrics.walk; if(w&&w.continuous_chebyshev_delta!=null&&w.trajectory_drift_sigma!=null){ const s=+w.trajectory_drift_sigma,c=+w.continuous_chebyshev_delta; return Math.max(0,(c-2*s)).toFixed(2)+' – '+(c+2*s).toFixed(2);} return null; } },
  { id: 'g-drift',     get: (st) => st.metrics&&st.metrics.drift!=null ? (Math.round(st.metrics.drift)+'%') : null },
  // g-cheby joined 2026-07-21 (all-measures contract): the walk carries continuous_chebyshev_delta
  // on every metal render — the tile just never had a getter, so it dashed forever.
  { id: 'g-cheby',     keep: true, get: (st) => { const w=st.metrics&&st.metrics.walk; return w&&w.continuous_chebyshev_delta!=null ? ((w.continuous_chebyshev_delta>=0?'+':'')+(+w.continuous_chebyshev_delta).toFixed(3)) : null; } },
...
var __lastTile = {};   // hoisted with HUD_MAP (TDZ incident: boot paintHud ran before this assignment)
...
function paintHud(st){
  ...
  if (st && /^SPLIT-/.test(String(st.id||''))) return;   // administrative states (SPLIT verdicts, by ID) own no Greeks — never blank the newest walk's HUD. Keyed on the ID, not walk-absence: the walk-absence heuristic also matched the BOOT/demo paint and silently skipped the modal's initialization (the empty-modal incident, 2026-07-21)
  for(const t of HUD_MAP){ const el=$(t.id); if(!el) continue; const v=t.get(st);
    // TILE-LEVEL OWNERSHIP (the after-loading blank, 2026-07-21): a Mode-A record carries drift
    // but no walk — it may paint drift, it may NOT blank the walk-owned tiles. A tile keeps its
    // last value when the incoming state simply doesn't own that measure; dash only ever means
    // 'no state on this tape has carried this measure yet'.
    if(v==null && t.keep && el.textContent && el.textContent!=='—') continue;
    ...
    // THE LABELED-DASH LAW, DYNAMIC HALF (2026-07-21): a null metric paints a dash WITH its
    // mechanism — this sealed state simply does not carry the metric. Same law as the static
    // build; a bare unexplained dash is the defect class the empties ratchet exists to kill.
    if(v==null){ const tile=el.closest('.tile'); if(tile && !tile.title) tile.title='— means this sealed state does not carry this metric (older states pre-date some instruments); step the scrubber to a newer state or run a fresh measurement'; }
  }
...
async function liveRustRender(){
  ...
  try {
    ...
    if(r.ok){
      ...
      // encircled is ALWAYS populated AND always an ENCIRCLED render (regions-chip.encircledPanel walks the
      // surface chain but never changes panel type — the 2026-08-04 "that is not the right panel type" fix).
      // When it encircled a surface other than tolerance the server says so in encircledSource; we PRINT it
      // rather than let the caption claim a tolerance read it did not take.
      ...
      const srcNote = (p, lane)=> (p && p.encircledFallback) ? lane+' encircled over the <b>'+p.encircledSource+'</b> surface (tolerance had 0 regions)' : '';
      const sn = $('encSrcNote'); if(sn){ const parts = [srcNote(a,'A'), srcNote(b,'B')].filter(Boolean); sn.innerHTML = parts.length ? '◎ '+parts.join(' · ') : ''; }
...
function initLoopControls(){
  var ttb = $('loadTickTriad'), tti = $('tickTriadInfo');
  if (ttb) ttb.onclick = function(){
    fetch('./tick-triad', {cache:'no-store'}).then(function(r){ return r.json(); }).then(function(t){
      if (!t || !t.inputs) { if (tti) tti.textContent = 'no harvest seals on the tape'; return; }
      var ti=$('tIntent'), tr=$('tReality'), tn=$('tNegative');
      if (ti) { ti.value = t.inputs.intent || ''; ti.dispatchEvent(new Event('input',{bubbles:true})); }
      if (tr) { tr.value = t.inputs.reality || ''; tr.dispatchEvent(new Event('input',{bubbles:true})); }
      if (tn) { tn.value = t.inputs.negative || ''; tn.dispatchEvent(new Event('input',{bubbles:true})); }
      if (tti) tti.textContent = t.id + ' · drift ' + (t.drift != null ? t.drift + '%' : '—') + ' · ' + (t.verdict || '') + (t.promise ? ' · ' + t.promise.settled + ' settled/' + t.promise.mass_bytes + 'B' : '');
    }).catch(function(){ if (tti) tti.textContent = 'serve not reachable (file:// page is air-gapped)'; });
  };
  ...
  poll(); setInterval(function(){ if(livePolling()) poll(); }, 5000);
}

// MASTER POLLING SWITCH — every timer on this page consults it before fetching (the 0.5s
// tape/viewport/version combo is the battery cost the checkbox names). Preference persists
// across reloads; intervals keep firing but return early when off, so re-enabling is instant.
function livePolling(){
  var el = $('livePoll');
  return !el || el.checked !== false;
}
(function(){
  var el = $('livePoll'); if(!el) return;
  try { if(localStorage.getItem('attest-live-poll') === '0') el.checked = false; } catch(e){}
  el.addEventListener('change', function(){
    try { localStorage.setItem('attest-live-poll', el.checked ? '1' : '0'); } catch(e){}
  });
})();
...
function renderGrowthCurve(){
  ...
  host.innerHTML = '<div style="border:1px solid #2a3350;border-radius:8px;padding:10px 12px;background:#0d1220">'
    ...
    + '<div class="dim" style="font-size:10.5px;line-height:1.5;margin-bottom:6px">The general model already knows <b>where</b> things go (<span style="color:#4aa3ff">routing flat at 100%</span> at every reef fraction) — that intelligence is free and everyone has it. What it cannot fake is the <b style="color:#46d369">playbook</b>: rules precision <b>climbs with reef-mass, byte by byte</b> — capability here is a purchased trajectory, not an emergent switch. That shape is the alpha: a curve with a public denominator is measurable exposure (what underwriters price) and a reproducible method (what open source audits) — the stated ideal stays the comparison target, countable either way.</div>'
...
if(/^https?:$/.test(location.protocol)){ setInterval(()=>{ if(livePolling()){ pollTape(); pollViewportConfig(); pollPageVersion(); } }, 500); }
// ── THE BOOT PAINT — runs LAST, after every declaration above (the TDZ cascade incident
// 2026-07-21: this block used to sit mid-script and each later let/const it reached through
// goToState → paintHud → liveRustRender killed the whole page at load, one variable at a time).
...
<!-- voice-meld-check: skip — this page embeds the reef corpus VERBATIM (byte-parity guarded); bare slot-labels here are quoted ground truth, not authored prose -->${milestonesSection()}${throughputSection()}${contextCaliperSection()}${ingestIntegritySection(intentResolved.cells, authorized)}${advancedModeSection()}${revertedSection()}${knockOnSection()}${narratorABSection()}${ratchetSection()}</div>${LENS_CAL}</body></html>`;
}
