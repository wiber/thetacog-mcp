...
import { appendPricedRow } from './ledger-attest.mjs';
...
export function computeSigmaLocalize({
  ...
  if (appendHistory) {
    try {
      ...
      // ONE DOOR (2026-08-09): rotation archives + the seal moves with the write — this was the
      // third hand-rolled slice(-200) rewrite behind the standing ROOT MISMATCH.
      appendPricedRow(line, historyPath, { cap: 200 });
...
<p style="font-size:11px;color:#5f6b78">rings: ${ringLine}</p>
  ...
  if (process.argv.includes('--email')) {
    ...
    execSync(`node ${resolve(REPO, 'scripts/email-artifact.mjs')} --html ${tmp} --no-attach --attach ${fp} --to you@example.com --to elias@thetadriven.com --from laboratory@thetadriven.com --subject ${JSON.stringify(subject)}`, { cwd: REPO, stdio: 'inherit' });
  }
  return r;
}
