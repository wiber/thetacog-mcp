...
export async function definerWalk144(startAnchors, { gridBits = null, maxDepth = 8, topK = 3, budget = 220, budgetMs = 600000, decay = 0.5, onHop = null } = {}) {
  ...
  try {
    ...
    // THE WALL-CLOCK BOUND IS A SAFETY VALVE, NOT A STOPPING RULE — and when it fires, the result is
    // NOT RECOMPUTABLE (2026-07-13). A walk cut short by elapsed-time depends on machine load, so the
    // same input yields a different matrix on a busy box than on an idle one. That silently moved the
    // demo's walk σ between 9.5 and 9.6 across three consecutive runs on ONE machine — while the same
    // output claimed "byte-identical every run". The placement coordinate was never affected (it is
    // work-bounded), but a receipt that carries ANY time-derived number cannot honour the recomputability
    // claim the whole product rests on. So: record whether the valve fired, and let the caller refuse to
    // present a tripped walk as recomputable. Never let it fail silently again.
    // Guard: tests/pmu/walk-sigma-recomputable.test.mjs.
  ...
  // timeBudgetTripped: the wall-clock valve cut the walk before its WORK bound (hops/frontier/depth)
  // was reached — i.e. this result is machine-load-dependent and NOT byte-reproducible. Anything that
  // puts this walk into a receipt must check this flag and refuse the recomputability claim if set.
  const timeBudgetTripped = frontier.length > 0 && hops < budget && (Date.now() - t0) >= budgetMs;
  return { heat, ply, hops, maxPly, matrix, mPly, timeBudgetTripped, elapsedMs: Date.now() - t0 };
}
