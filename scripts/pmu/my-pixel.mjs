...
const entry = {
  ...
  // C308j: the ONE date this receipt carries — when THIS repo minted it. The big-map crawler
  // (map-crawl.mjs) reads this straight through into tape/map.json so the adoption board can
  // report a first/last minted date across every foreign repo, never a re-derived guess.
