...
// THE ENTRY: expand a coord into { coord, canonical, hat, domain, guessed, vocab, name }.
//   name = "<canonical> → <domain> · <vocab head>"  (domain guessed → "~<domain>")
//   hat  = the worn ROLE ("Operator×Counsel") — the written-out hat name (operator 2026-08-06:
//          hats get names on every surface; the ONE place is here, so email/receipt/page all inherit).
...
export function expandCoordName(coord, { vocabWords = 4 } = {}) {
  ...
  const hat = hatName(key);
  const exact = reef().byCoord.get(key);
  const hit = exact ? { domain: exact, dist: 0 } : nearestDomain(key);
  if (!hit) return { coord: key, canonical, hat, domain: null, guessed: false, vocab: [], name: canonical };
