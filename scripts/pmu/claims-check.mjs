...
function latestStudy() {
  ...
  const f = existsSync(dir) ? readdirSync(dir).filter((x) => /\.json$/.test(x)).sort().pop() : undefined;
  if (!f) throw new Error(`NO_STUDY_DATA: no *.json under ${dir} — bundle/reinstall the package (data/pmu/study/ must ship real study files, not just .gitkeep)`);
