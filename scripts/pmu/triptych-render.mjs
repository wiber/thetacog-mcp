...
// ── C18 — ONE PLACE FOR THE TWO CONSTANTS THAT DECIDE A REFUSAL AND A BLOCK ─────────────────────
// LIT_MASS_FLOOR was a function-local const here AND an exported const in scripts/vna/self-lane-panel.mjs.
// Two homes for the number that decides whether a panel REFUSES: if one moves, one surface refuses
// and the other renders on the same input, and the receipt says both. 24 significant cells ≈ two
// blocks' worth on each side; calibration may raise it, never remove it.
export const LIT_MASS_FLOOR = 24;

// dominantBlocks was byte-identical in scripts/pmu/tolerance-panel.mjs:54 and
// scripts/pmu/triptych-build.mjs:80 — the same single line twice, which is a copy waiting to diverge.
export function dominantBlocks(f32, blockOf) {
  if (!f32) return [];
  const bm = new Array(16).fill(0);
  for (let i = 0; i < 20736; i++) bm[blockOf((i % 144) % 144)] += f32[i];
  return [...bm.keys()].filter((b) => bm[b] > 0).sort((a, b) => bm[b] - bm[a]).slice(0, 4);
}
...
export function decodeDeltaThreeColourEdges(im, rm, killTolerancePct = 15, { k = 1.0 } = {}) {
  ...
  // ── THE LIT-MASS FLOOR (the aperture check; S2 closes, 2026-08-14) ─────────────────────────
  // A 52-word off-lane document once scored 0/0/0 · off 0% · ok — a full-length adversary reading
  // as a flawless submission, because thin input builds no lattice mass and absence classified as
  // a clean pass. Below the floor the panel REFUSES to grade: offPct null (so every downstream
  // fail-closed gate reads UNMEASURED, never IN_LANE), counts zeroed, refusal named. The floor is
  // a deliberately conservative constant (24 significant cells ≈ two blocks' worth on EACH side);
  // calibration may raise it, never remove it. "The cheapest strategy must never be silence."
  let isMass = 0, rsMass = 0;
  for (let i = 0; i < CELLS; i++) { if (is[i]) isMass++; if (rs[i]) rsMass++; }
  if (isMass < LIT_MASS_FLOOR || rsMass < LIT_MASS_FLOOR) {
    return { rgba, green: 0, amber: 0, red: 0, tooMany: false, offPct: null, perRow, pattern: 'REFUSED',
      domBlockCount: domBlocks.length, refused: true, litMass: { intent: isMass, reality: rsMass, floor: LIT_MASS_FLOOR },
      refusal: `LIT-MASS FLOOR — intent ${isMass} / reality ${rsMass} significant cells (floor ${LIT_MASS_FLOOR}); too thin to grade. UNMEASURED, never a pass.` };
  }
...
${tilesDump}${ctxRow}</div>`;
  ...
  // rgbas: the per-panel 144×144 SURFACES by id (references, not copies). The encircled panel must be
  // able to encircle a surface OTHER than tolerance when tolerance has no regions — without that, callers
  // "fell back" by substituting a raw PNG of the wrong panel type (see regions-chip.encircledPanel).
  const rgbas = Object.fromEntries(panels.map(p => [p.id, p.rgba]));
  return { dataHtml: mk(dataUris), cidHtml: mk(pngs.map(p => `cid:${p.name}`)), pngs, rgbas, tol, friction, preWalk };
}
