...
// THE CANONICAL SENSOR, IMPORTED — never re-declared here (bf-5150). This file used to carry its own
// copy of ncd; the audit in docs/architecture/narrow-channel-posture-spec.md §14.2 named it as one of
// the two remaining seams in "one pipeline". The local copy was byte-identical to the canonical across
// empty / unicode / 5KB inputs (verified before the swap), so this changes no number — it removes the
// fork that could drift. Import cost measured at 6ms; rule-grab guards its own CLI entry so importing
// it runs nothing. Registered as `sensor.distance` canonical in src/lib/pmu/pipeline-registry.mjs.
import { ncd } from '../cog/rule-grab.mjs';

const REPO = resolve(dirname(fileURLToPath(import.meta.url)), '..', '..');
const SN = (() => { try { return JSON.parse(readFileSync(resolve(REPO, 'data/pmu/snippet-library-144.json'), 'utf8')); } catch { return []; } })();
export const cellText = (c) => String((SN.find((x) => String(x.coord) === String(c)) || {}).snippet || '');
