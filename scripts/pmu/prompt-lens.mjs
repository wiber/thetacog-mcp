...
const DOMAIN_PATHS = (() => { try {
  const rows = JSON.parse(execFileSync('sqlite3', ['-json', resolve(REPO, 'data/thetacoach.db'),
    "SELECT domain, paths FROM lens_rule_meta WHERE paths IS NOT NULL;"], { encoding: 'utf8' }) || '[]');
  const m = new Map();
...
export function routeToDomain(text) {
  ...
  const scored = REEF.map((d) => {
    ...
    const hits = vocab.reduce((n, v) => n + (words.has(v) || t.includes(v) ? 1 : 0), 0);
...
export async function boundaryFromStubSpec(stub, { pairLib = loadPairLib(), radius = BOUNDARY_RADIUS, walkTimeoutMs = WALK_TIMEOUT_MS } = {}) {
  ...
  return { center, block: [br, bc], radius, box, sigma, sensor, domain, template, domainRules, walk: dom ? 'repo-domain' : 'ballistic-seed', compressMs, seedGzipUs, walkMs, walkMetalMs, routeUs, walkPlies, walkFillPct, walkWalksPerSec, walkSaturated, walkProvenance };
}
...
export function retrieveRules(boundary, { db = DB, cap = MAX_RULES, intentText = '' } = {}) {
  let topWhy = '';
  ...
  if (intentText && core.length > 1) {
    ...
    const ruleToks = core.map((r) => new Set(String(r).toLowerCase().split(/[^a-z0-9]+/).filter(w => w.length > 3).map(stem)));
    const df = new Map();
  ...
  try {
    const g = execFileSync('sqlite3', ['-json', resolve(REPO, 'data/thetacoach.db'), "SELECT gate, value FROM tc_pipeline_gates WHERE gate IN ('lens-rules-char-budget','lens-min-core-rules') AND enabled=1;"], { encoding: 'utf8' }).trim();
  ...
  return { rules, sqlMs, sortUs, topWhy, coreAvailable, note: `${core.length} core + ${perimeter.length} perimeter → ${rules.length}/${cap} (${chars}≤${charBudget}ch)` };
}
...
export function renderReceipt(result) {
  ...
  const line3 = `${rulesSeg} · 🎯 sort-quality: ${r.sortQuality} · reef-health ${rh}% · reef ${reefCh} ch`;
...
export async function lensPrompt(prompt) {
  ...
  const base = { prompt, stub, boundary, retrieval, ms, timings, within_budget: ms <= LATENCY_BUDGET_MS };
