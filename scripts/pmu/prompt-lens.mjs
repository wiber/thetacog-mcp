...
export async function boundaryFromStubSpec(stub, { pairLib = loadPairLib(), radius = BOUNDARY_RADIUS, walkTimeoutMs = WALK_TIMEOUT_MS } = {}) {
  ...
  let center, domain = null, template = '', domainRules = null, compressMs = 0, placementMode = 'route', placementFit = null, rustLens = null;
  if (dom) { center = dom.coord; domain = dom.domain; template = dom.template || ''; domainRules = dom.rules || []; }
    ...
    rustLens = (() => { try { const w = lensWalk(text, { source: (process.env.LENS_SESSION_ID || process.env.CLAUDE_SESSION_ID) ? 'turn' : 'lens-cli', bulk, repoRoot: REPO, timeoutMs: 800 }); return w.row.admissible ? w.result : (w.result ? { ...w.result, _unmeasured: true } : null); } catch { return null; } })();   // a CLI/test run is not a turn — the denominator stays honest (zero-bias tape)
    if (rustLens && rustLens._unmeasured) { center = null; placementMode = 'unmeasured'; placementFit = rustLens.seed_fit || null; }   // the walk ran the ladder and nothing was admissible — no lane, say so
    else if (rustLens) { center = rustLens.pixel; placementMode = bulk ? 'rust-lens-matched-momentum' : 'rust-lens-matched'; placementFit = rustLens.seed_fit || null; }
  ...
  if (dom && !rustLens) {
    try {
      ...
      const w = lensWalk(text, { source: (process.env.LENS_SESSION_ID || process.env.CLAUDE_SESSION_ID) ? 'turn' : 'lens-cli', bulk: laneBulk || null, lane: dom.domain, repoRoot: REPO, timeoutMs: 800, extra: { routed: dom.domain, routed_center: dom.coord } });
      if (w.result && w.result.seed_fit) placementFit = w.result.seed_fit;   // the Fit line shows the null test on routed turns too
  ...
  const _placement = { mode: placementMode, fit: placementFit };
...
export function renderReceiptCompact(result) {
  ...
  { const pl = result.boundary?.placement; const fit = pl?.fit;
    const verdict = (x) => x.better_than_random ? 'admissible' : `below threshold (needs gain ≥ ${FIT_MIN_GAIN}, z ≥ 2) — placement UNMEASURED, rules served as candidates`;
...
export async function lensPrompt(prompt) {
  ...
  try { base.tokenCost = recordCost({ injection: base.lensedContext, receipt: renderEchoedReceipt(base) }); } catch { /* best-effort */ }
  return base;
}
