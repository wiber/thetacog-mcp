...
export async function ramRunnerReadout() {
  ...
  const RUST = resolvePmuBinary(REPO_ROOT);
  ...
  try {
    ...
    rust = {
      available: true, binary: RUST.replace(REPO_ROOT + '/', ''),
...
const server = createServer(async (req, res) => {
  try {
    ...
    // C88 vna.stream — READ-ONLY SSE of the flight tape + witness file, every row AS WRITTEN (the raw ndjson line, signed bytes
    // intact) with its inclusion proof beside it; GET only (any other method → 405), loopback only (this server's bind), no model
    // anywhere on the path. The tailing + framing live in scripts/vna/stream.mjs so the guard drives them without this port.
    if ((req.url || '').split('?')[0] === '/vna.stream') {
      try { const { streamRoute } = await import(resolve(HERE, '..', 'vna', 'stream.mjs')); if (streamRoute(req, res)) return; }
      catch (e) { res.writeHead(500, { 'content-type': 'application/json' }); res.end(JSON.stringify({ error: String(e && e.message || e) })); return; }
    }
