...
async function run() {
  ...
  if (process.env.DEMO_TH_REC !== '1') {
    // Never hand a stranger a dead end: the old parallel report generator stays
    // retired, but the documented `pmu-demo` entry now routes onto the one
    // canonical pipeline (`prove` — the first-run self-proof) instead of exiting.
    console.log('pmu-demo folded into the canonical proof — running `prove` (the first-run self-proof).');
    console.log('(the retired demo-th-rec report generator re-arms only with DEMO_TH_REC=1)');
    console.log('');
    await import('./pmu/prove.mjs');
    return;
  }
