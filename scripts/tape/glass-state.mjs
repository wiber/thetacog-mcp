...
//   question.leverage  the persisted UPSTREAM/DOWNSTREAM altitude and the fanout/translation/
//                   leverage numbers behind it — read out of questions.ndjson with the ranking's own
//                   timestamp, so a stale reading is visibly stale. Absent = a named unmeasured.
...
// The leverage/altitude reading is READ from the ranking record, never computed here — the walk
// that produces it is minutes long and offline (see leverage-reading.mjs). One door for the words,
// so the JSON an agent reads and the HTML the operator reads carry the SAME sentence.
import { leverageReading } from './leverage-reading.mjs';
...
export async function questionFor(slug) {
  ...
  const lev = leverageReading(recAsk, { askedAt: askTs, slug });
  return {
    id: recAsk.id,
    question: recAsk.question,
    // THE FORK, SURFACED (2026-08-24). A minted question is a binary — its answers[] carry the two
    // named branches — but only the prose sentence reached the state, so the reader met a 57-word
    // single sentence and had to parse the choice back out of it. The labels were computed for the
    // leverage reading and then dropped. Carrying them lets both readers lead with WHAT IS BEING
    // DECIDED and keep the qualifying clause underneath, which is the same reordering the prose
    // gate asks for: the decision first, its conditions second.
    fork: Array.isArray(recAsk.answers) ? recAsk.answers.map((a) => a.label).filter(Boolean) : [],
    askedAt: askTs,
    // WHY THE NUMBERS RIDE ON THE ANSWERS. The steering failure this reading exists to catch is a
    // question drifting DOWN into technical specification, and that is a per-ANSWER property: a
    // "Redis or Postgres" answer places on a leaf execution cell and governs almost nothing, while a
    // values-tradeoff answer places upstream and governs a large downstream set. An aggregate alone
    // would hide which branch is the leaf, so both are carried.
    answers: (recAsk.answers || []).map((a, i) => ({
      label: a.label,
      coord: a.coord ?? 'unplaced',
      fanout: lev.answers[i]?.fanout ?? null,
      translation: lev.answers[i]?.translation ?? null,
      leverage: lev.answers[i]?.leverage ?? null,
      leverageUnmeasured: lev.answers[i]?.unmeasured ?? null,
    })),
    // The question-level altitude: a value, or a sentence naming what is missing. Never a 0.
    leverage: {
      measured: lev.measured,
      aggregate: lev.aggregate,
      aggregateSource: lev.aggregateSource,
      aggregateBasis: lev.aggregateBasis,
      altitude: lev.altitude,
      altitudeWords: lev.altitudeWords,
      computedAt: lev.computedAt,
      unmeasured: lev.unmeasured,
    },
    retirement,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.argv.includes('--json')) {
    ...
    if (q.unmeasured) {
      console.log(`  question: UNMEASURED — ${q.unmeasured}`);
    } else {
      // Fork first, sentence second. A reader deciding whether to engage needs the choice, not the
      // conditions under which the choice arises.
      const fork = (q.fork || []).length >= 2 ? `${q.fork.join(' or ')}?` : '';
      console.log(`  question: ${q.id}${fork ? `  ·  ${fork}` : ''}`);
    ...
    // The altitude in the terminal too — an agent driving the CLI must be able to see the same
    // verdict the operator sees on the glass, in the same step, without curling the page.
    if (!q.unmeasured) console.log(`  altitude: ${q.leverage.altitudeWords || q.leverage.unmeasured}`);
