...
export function createSession({ slug, sources = [] } = {}) {
  // A CALLER-SUPPLIED SLUG IS SANITIZED TOO. slugify() is idempotent on real slugs
  // ('gddadwill' -> 'gddadwill'), so this is invisible in normal use — but it is the only
  // thing standing between a caller that passes a PATH and a session directory written
  // outside the namespace. One did: .thetacog/tape-sessions/Users/thetacoach/GitHub/…/
  // GDDadwill.txt/session.json, a filesystem tree mirrored inside the sessions dir. A slug
  // with '..' in it would leave the repo entirely.
  const resolvedSlug = slug ? slugify(slug) : (sources[0] ? slugify(sources[0]) : 'session');
...
/**
 * extractionSummary(atoms, session) -> string  — PURE. What actually produced this tape's atoms,
 * counted off the ledger. It lives HERE, beside the ledger it reads, because cli.mjs executes a
 * verb the moment it is imported — a guard could never hold the function to the ledger from there.
 *
 * A session with no atoms yet cannot be counted, and must not be guessed at: it falls back to the
 * session's own declared value, clearly marked as a declaration and not a measurement.
 */
export function extractionSummary(atoms, session = {}) {
  const counts = new Map();
  for (const a of atoms || []) {
    const e = (a && a.extractor) || 'unlabelled';
    counts.set(e, (counts.get(e) || 0) + 1);
  }
  if (!counts.size) {
    const declared = session.extraction ? String(session.extraction).split(' — ')[0] : 'none';
    return `${declared} (declared — no atoms walked yet, nothing to count)`;
  }
  const rows = [...counts.entries()].sort((a, b) => b[1] - a[1]);
  if (rows.length === 1) return rows[0][0];
  return rows.map(([e, n]) => `${e}:${n}`).join(' + ');
}
