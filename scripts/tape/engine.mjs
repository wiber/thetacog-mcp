...
export function ingestTranscript(filePath, offset = 0) {
  return new Promise((resolvePromise) => {
    execFile(
      ...
      { ...PMU_SPAWN_DEADLINE, maxBuffer: 256 * 1024 * 1024 },
      (err, stdout, stderr) => {
        if (err) {
...
/**
 * prefetchKey(turn, primer) -> string  — PURE. What makes an ALREADY-COMPUTED extraction still
 * valid at commit time.
 *
 * WHY IT EXISTS. run() starts the extraction for turn N+1 while turn N is still committing. Between
 * those two moments the operator can apply a steer, which either splices new turns into the queue
 * (changing which turn sits at the cursor) or adds prose to the primer (changing what the extraction
 * should have been told). Committing a stale extraction would place an atom against words that are
 * no longer the words at that position — a receipt that says the wrong thing with full confidence,
 * which is the one failure mode the whole tape exists to refuse. The key pins the turn's identity
 * AND the primer; a mismatch discards the prefetch and extracts live. Never widen this key to make
 * more prefetches "hit" — a missed hit costs one model call, a false hit corrupts the ledger.
 */
export function prefetchKey(turn, primer) {
  if (!turn) return null;
  return JSON.stringify([turn.idx ?? null, turn.source ?? null, turn.startLine ?? null, turn.endLine ?? null, (turn.text || '').length, String(primer || '')]);
}
...
export async function step(slug, opts = {}) {
  ...
  // R0 IS ABOUT WHAT ONE CALL SEES, NOT WHEN IT WAS SCHEDULED. The law at GDDadwill.txt L1328 bans
  // processing the transcript in one pass — one turn per model call. It says nothing about the call
  // for turn N+1 being in flight while turn N commits, and that costs nothing in fidelity: the
  // prompt still carries exactly one turn. `opts.pre` is run()'s already-started extraction of THIS
  // turn, honoured only when prefetchKey still matches (see above), never blindly.
  const preKey = prefetchKey(turn, primer);
  const atomResult = (opts.pre && preKey && opts.pre.key === preKey && opts.pre.atomResult)
    ? opts.pre.atomResult
    : await atomizeTurn(turn, { session, primer, repoRoot: opts.repoRoot || REPO, model: opts.model });
...
//
// THE LOOKAHEAD, and why it is not a violation of R0.
//
// Measured 2026-08-21 on the live GDDadwill walk: 207 turns, one `claude -p --model sonnet` spawn
// each, ~40s per spawn, issued STRICTLY ONE AT A TIME by the `while` loop that used to live here.
// That is ~2.3 hours of wall clock in which exactly one model call is ever in flight — the shape
// CLAUDE.md's "QWEN IS BANNED FROM ACTIVE TASKS · PARALLEL SONNET SUBAGENTS INSTEAD" rule names as
// banned ("a sequential for/while loop with an await on a model call inside it"). tests/tape/
// no-sequential-model-loop.test.mjs guards atomizer.mjs — which is compliant, it has a bounded pool
// — and the banned loop had simply moved one file over into engine.mjs, where nothing scanned it.
//
// The fix keeps BOTH invariants that actually matter and trades neither for speed:
//   R0 (one turn per model call)  — every prefetched call carries exactly one turn's text. The
//                                   containment boundary is the CONTENT of a call, not its start time.
//   Turn-ordered commit           — the ledger loop below is still strictly sequential, one turn at
//                                   a time, in queue order. Atom ids (nextAtomId reads the live
//                                   ledger), home-coordinate election, laneDrift backfill, and the
//                                   vega AUC series are all order-dependent, so they stay serial and
//                                   produce byte-identical output to the old loop.
// What changes is only WHEN the extraction for turn N+k is started. Bounded waste: at most
// `lookahead - 1` extractions may be paid for and then discarded if the walk stops, pauses, or a
// steer invalidates them. That is the price, it is bounded, and it is named here rather than hidden.
const DEFAULT_LOOKAHEAD = Math.max(1, Number(process.env.TAPE_LOOKAHEAD) || 4);
...
export async function run(slug, {
  ...
  const results = [];
  let n = 0;
  const width = Math.max(1, Number(lookahead) || 1);
  const inflight = new Map(); // prefetchKey -> Promise<atomResult | {__prefetchError}>

  // Start (up to) `width` extractions covering the cursor and the turns just after it. Each call
  // receives ONE turn. Keys already in flight are never re-issued, so this is idempotent per tick.
  const prime = async (session, queue, primer) => {
    const room = Math.min(width, Number.isFinite(maxTurns) ? Math.max(0, maxTurns - n) : width);
    const live = new Set();
    for (let k = 0; k < room; k++) {
      const t = queue[session.cursor + k];
      if (!t) break;
      const key = prefetchKey(t, primer);
      if (!key) continue;
      live.add(key);
      if (inflight.has(key)) continue;
      inflight.set(key, atomizeTurn(t, { session, primer, repoRoot: repoRoot || REPO, model })
        .catch((e) => ({ __prefetchError: e && e.message ? e.message : String(e) })));
    }
    // Drop anything the window has moved past or a steer invalidated — the promise is already
    // running and its result is simply discarded; it must not accumulate in the map for the
    // lifetime of a 200-turn walk.
    for (const key of [...inflight.keys()]) if (!live.has(key)) inflight.delete(key);
  };
  ...
  while (n < maxTurns) {
    ...
    const queue = await getTurnQueue(slug, session);
    const primer = buildPrimer(session);
    await prime(session, queue, primer);

    // Hand step() the extraction for the turn AT the cursor, if we have one. step() re-derives the
    // key itself and refuses a stale hand-off, so this is a hint, never an instruction.
    const key = prefetchKey(queue[session.cursor], primer);
    let pre = null;
    if (key && inflight.has(key)) {
      const v = await inflight.get(key);
      inflight.delete(key);
      if (v && !v.__prefetchError) pre = { key, atomResult: v };
    }

    const r = await step(slug, { ...(pre ? { pre } : {}), model, repoRoot });
    results.push(r);
    n++;
