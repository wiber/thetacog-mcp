...
// extractionSummary lives in store.mjs — it is a statement ABOUT the ledger, so it belongs beside
// the ledger, and a guard can import it there without this file executing a verb on import.
import { extractionSummary } from './store.mjs';
...
if (verb === 'status') {
  ...
  const payload = {
    ...
    // EXTRACTION IS COUNTED FROM THE LEDGER, NEVER READ FROM session.json.
    //
    // walk-spine.mjs stamps `extraction: 'deterministic-stub — the LLM atomizer lane is not yet
    // built'` into session.json ONCE, when the LLM-free spine lays the session down. The engine
    // walk that follows appends every atom with its own `extractor` field — and on the live
    // GDDadwill session that field read `atomizer.mjs` on all 160 atoms while this status line
    // was still printing "the LLM atomizer lane is not yet built". A write-once field describing
    // work that had not happened yet went on describing work that had.
    //
    // The direction of the error does not excuse it. An operator reading the status would report
    // real extractions as stub output, which is the same class of failure as the reverse and lands
    // in the same place: the status is a receipt, and a receipt that contradicts the ledger it
    // summarizes is worse than no receipt. So it is DERIVED — counted off the atoms themselves —
    // and it names every extractor present rather than collapsing a mixed session to one word,
    // because a session walked partly by the spine and partly by the atomizer is exactly the state
    // an operator most needs to see.
    extraction: extractionSummary(atoms, S),
  };
  out(payload, () => {
    console.log(C.b(`\n  tape · ${slug}`));
    ...
    if (payload.extraction !== 'atomizer.mjs') console.log(`  ${C.y('note')}      extraction = ${payload.extraction}`);
    console.log('');
  });
  process.exit(0);
}
