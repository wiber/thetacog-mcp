...
// 2026-09-24: CLAUDE.md promised this door searched the MOVED rules by content, and it did not: the corpus was three
// hand-typed files, so every docs/architecture/claude-rules/*.md (voice-and-pitch, the ratchet invariants, …) and the voice
// definitions in docs/strategy/the-holden-voice.md (Rule 13: the paradox voice solves the credit problem; the champagne voice is
// the shadow print, effort hidden for status) were unreachable. A /steer section then defined both voices as tones. The
// claude-rules files are LISTED from the folder, never typed, so a newly moved rule file is searched the day it lands.
const RULES_DIR = 'docs/architecture/claude-rules';
export const DEFAULT_CORPUS = [
  'CLAUDE.md',
  'docs/architecture/anti-rules-ledger.md',
  'docs/architecture/narrow-channel-posture-spec.md',
  ...(() => { try { return readdirSync(resolve(REPO, RULES_DIR)).filter((f) => f.endsWith('.md') && !f.startsWith('_')).sort().map((f) => `${RULES_DIR}/${f}`); } catch { return []; } })(),
  'docs/strategy/the-holden-voice.md',
];
