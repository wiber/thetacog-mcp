...
import { loadOrCreateHostKeys, sealReceipt, HOST_PRIVATE_KEY_PATH } from '../pmu/receipt-crypto.mjs';
...
// C97e NO ROW WITHOUT A SIGNATURE — the ONE key resolver for the tape. A row with a room signs with that room's derived key
// (signed_as 'room'); a row with no room signs with the LOCAL key — the host key itself, the one every room key is derived
// from, generated once under ~/.thetacog/pmu/keys (0600), never inside the repo (signed_as 'local'). Never a null signature
// for want of a room: the 188 unsigned rows of 2026-09-19 (C97) were this path refusing to sign. The fingerprint is C44's
// sha256(raw pubkey)[:16], the same 16 hex the notary indexes accounts by.
export const fingerprintOfHex = (pubkeyHex) => createHash('sha256').update(Buffer.from(String(pubkeyHex), 'hex')).digest('hex').slice(0, 16);
export function hostIdentity() {
  const k = loadOrCreateHostKeys();
  return { room: null, signed_as: 'local', privateKey: k.privateKey, publicKey: k.publicKey, pubkey_hex: k.pubkey_hex, fingerprint: fingerprintOfHex(k.pubkey_hex), keyPath: HOST_PRIVATE_KEY_PATH };
}
export function signerFor(room) {
  if (!room) return hostIdentity();
  const id = roomIdentity(room);
  return { ...id, signed_as: 'room', fingerprint: fingerprintOfHex(id.pubkey_hex), keyPath: HOST_PRIVATE_KEY_PATH };
}
