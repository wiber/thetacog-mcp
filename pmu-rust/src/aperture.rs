...
pub struct Row {
    ...
    pub truncated: bool,
    // C115z (operator: "we should see which parts of what files are aperture receipts") — WHERE
    // in the text handed to pick_window the kept window sits, so a receipt can say "bytes [a,b) of
    // this file's reality text" instead of only "N of M bytes, somewhere". usedStart/usedEnd are
    // byte offsets into the text this Doc carried (post-commentProse for code, raw for prose);
    // chunkSha is sha256 of the kept window's own bytes, so a chunk is independently re-verifiable
    // without re-running the walk — hash what was read, not what could have been read.
    #[serde(rename = "usedStart")] pub used_start: usize,
    #[serde(rename = "usedEnd")] pub used_end: usize,
    #[serde(rename = "chunkSha")] pub chunk_sha: String,
}
...
// Returns the (start, end) byte offsets INTO `text` after boundary trimming — C115z needs the real
// offsets that were kept, not just the substring, so a receipt can say which span of the file this is.
fn window_bounds(text: &str, at: usize, budget: usize) -> (usize, usize) {
    let b = text.as_bytes();
...
fn window(text: &str, at: usize, budget: usize) -> &str {
    let (start, end) = window_bounds(text, at, budget);
    &text[start..end]
}
...
pub(crate) fn pick_window(text: &str, budget: usize) -> (String, usize, usize) {
    ...
    let mut best_bounds = (0usize, 0usize);
    ...
    for i in 0..CANDIDATES {
        ...
        if d > best_density { best_density = d; best = w.to_string(); best_bounds = (start, end); }
    }
    if best.is_empty() {
        let (s, e) = window_bounds(text, 0, budget);
        (text[s..e].to_string(), s, e)
    } else {
        (best, best_bounds.0, best_bounds.1)
    }
}
...
pub fn match_aperture(intent: &[Doc], reality: &[Doc], tolerance: f64) -> Out {
    ...
    let mut cut = |docs: &[Doc], side: &str, raw: usize, rows: &mut Vec<Row>| -> String {
        ...
        for d in docs {
            ...
            let (used, used_start, used_end) = pick_window(&d.text, budget);
            let chunk_sha = hex_sha256(used.as_bytes());
            rows.push(Row { side: side.to_string(), path: d.path.clone(), raw_bytes: len,
                used_bytes: used.len(), share_pct: (share * 1000.0).round() / 10.0,
                truncated: used.len() < len, used_start, used_end, chunk_sha });
            parts.push(used);
        }
        parts.join("\n")
    };
