...
/// ALL user prompts in the transcript, same filter as first_user_prompt (system-reminders
/// stripped, >40 UTF-16 code units) but collecting every match instead of returning on the
/// first — the operator's own prompts, in order, are the real intent corpus. ADDITIVE: this is
/// a new function alongside first_user_prompt, which is untouched and keeps returning exactly
/// what it always has (the first entry of this same list, when non-empty).
pub fn all_user_prompts(path: &str) -> std::io::Result<Vec<String>> {
    let content = fs::read_to_string(path)?;
    let mut out = Vec::new();
    for line in content.split('\n').filter(|s| !s.is_empty()) {
        let obj: Value = match serde_json::from_str(line) {
...
mod tests {
    ...
    #[test]
    fn all_user_prompts_returns_every_qualifying_prompt_in_order() {
        // Fixture has ONE qualifying user turn (L_USER_SHORT is too short, filtered). Add a
        // second qualifying user line to prove multiple prompts accumulate, in file order, and
        // that all_user_prompts()[0] equals first_user_prompt() exactly — additive, not divergent.
        let second_user = r#"{"type":"user","message":{"role":"user","content":"This is the second real operator prompt, also well over forty characters."}}"#;
        let body = format!(
            "{}\n{}\n{}\n{}\n{}\n",
            L_TITLE, L_USER_SHORT, L_USER_LONG, L_ASST, second_user
        );
        let path = std::env::temp_dir()
            .join(format!("pmu-transcript-{}-allprompts.jsonl", std::process::id()));
        fs::write(&path, &body).unwrap();

        let all = all_user_prompts(path.to_str().unwrap()).unwrap();
        assert_eq!(all.len(), 2);
        assert_eq!(all[0], "Build the streaming transcript reader for the chip");
        assert_eq!(
            all[1],
            "This is the second real operator prompt, also well over forty characters."
        );

        // Never diverges from first_user_prompt on the same file.
        let first = first_user_prompt(path.to_str().unwrap()).unwrap();
    ...
    #[test]
    fn all_user_prompts_is_empty_when_no_prompt_qualifies() {
        let path = write_fixture("allprompts-empty", false);
        // write_fixture's only user lines are L_USER_SHORT (too short) and L_USER_LONG (qualifies) —
        // use a body with ONLY the short one to prove empty-vec, not empty-string, is returned.
        let body = format!("{}\n{}\n", L_TITLE, L_USER_SHORT);
        fs::write(&path, &body).unwrap();
        let all = all_user_prompts(&path).unwrap();
