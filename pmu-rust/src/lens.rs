...
pub fn shuffle_words(s: &str) -> String {
...
pub fn matched_seed_parts(text: &str, parts: &[(Option<usize>, String)], targets: &[String], ap: &SeedAperture) -> (Vec<f64>, SeedFit) {
    let prompt = text;
    ...
    let null_doc = shuffle_words(&intent);
    let null_z = node_gzip_len(null_doc.as_bytes());
    let mut fine: Vec<(f64, usize)> = region.iter().map(|&i| {
        let (di, diz, nd, nz) = match variant(i) { Some(v) => { let z = node_gzip_len(v.as_bytes()); let n = shuffle_words(&v); let nz = node_gzip_len(n.as_bytes()); (v, z, n, nz) } None => (intent.clone(), d_fz, null_doc.clone(), null_z) };
...
pub fn run(args: &[String]) {
    ...
    let (scores, seed_fit) = if seed_mode != "matched" { (lit_scores(&text, &lib.targets), None) } else {
        ...
        let mut record = |label: &str, mass_chars: usize, hats: usize, s: Vec<f64>, f: SeedFit, txt: &str, span: &Value, best: &mut Option<(Vec<f64>, SeedFit, String, String, Value)>| -> bool {
            ...
            attempts.push(json!({ "label": label, "mass": mass_chars, "hats": hats, "thread_prompts": span.get("prompts").cloned().unwrap_or(Value::Null), "gain": (f.gain * 1e4).round() / 1e4, "z": (f.z_fine * 100.0).round() / 100.0, "ok": f.better_than_random, "grip": grip, "pixel": top }));
            ...
            if take { *best = Some((s, f, label.to_string(), txt.to_string(), span.clone())); }
            ok
        };
        ...
        'stages: for k in 0..4usize {
            let (stage_text, sp) = if k == 0 { (text.clone(), span_of(&intent_span)) } else {
                if session_id.is_none() || last_prompts >= THREAD_MAX_PROMPTS { break; }
            ...
            let mut ladder: Vec<(String, String)> = if thin_line && !no_ladder && k == 0 { bulk_ladder(&stage_text, &bulk, lane_name.as_deref(), &reef) } else if k > 0 && thin_line && !no_ladder { bulk_ladder(&stage_text, &bulk, lane_name.as_deref(), &reef).into_iter().take(2).collect() } else { Vec::new() };
            if ladder.is_empty() { ladder.push((if bulk.is_empty() { "none".to_string() } else { "given".to_string() }, bulk.clone())); }
            let mut latched = false;
            for (label, mass) in &ladder { steps += 1; let (s, f) = matched_seed_with(&stage_text, mass, &lib.targets, &ap); if record(&format!("{}{}", pre, label), mass.chars().count(), 0, s, f, &stage_text, &span_json, &mut best) { latched = true; } }
            ...
            while passes_here < guided_max && !lanes.is_empty() {
                ...
                for (label, (lr, lc), mass) in &lanes { if walked.iter().any(|(r, c)| (r - lr).abs() <= 2 && (c - lc).abs() <= 2) && seen.insert(label.clone()) { parts.push((None, mass.clone())); } }
                // HATS: the snippets seated at the walked cells, each tagged with its target (left out when that target is scored)
                let mut hats = 0usize;
                for &i in &walked_idx { let h = cut(&lib.targets[i], 600).to_string(); if !h.trim().is_empty() { parts.push((Some(i), h)); hats += 1; } }
                ...
                let (s, f) = matched_seed_parts(&stage_text, &parts, &lib.targets, &ap);
                if record(&format!("{}guided:{}", pre, passes_here), mass_chars, hats, s, f, &stage_text, &span_json, &mut best) { latched = true; }
            }
            if latched { break 'stages; }   // a latched stage ends the backward travel; grip already chose among this stage's admissible rungs
        }
        let (bs, bf, winner, wtext, wspan) = best.expect("at least one rung");
        text = wtext; intent_span = wspan;
        let wgrip = grips.iter().find(|(l, _)| *l == winner).map(|(_, g)| *g);
        ratchet_json = json!({ "steps": steps, "guided_passes": guided_passes, "thread_stages": stages, "winner": winner, "admissible": bf.better_than_random, "grip": wgrip, "admissible_rungs": attempts.iter().filter(|a| a.get("ok").and_then(|o| o.as_bool()).unwrap_or(false)).count() });
        (bs, Some(bf))
    };
...
mod hat_tests {
    ...
    fn a_hat_never_scores_its_own_target() {
        ...
        let leaky = matched_seed_parts(prompt, &[(None, targets[1].clone())], &targets, &ap).0;   // untagged = the leak
        assert!(leaky[1] > without[1], "an UNTAGGED hat would inflate its own target — the tag is what fences the leak");
    }
}
