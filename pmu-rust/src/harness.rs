    ...
    for budget in 0..=(text.len() + 8) {
        ...
        match r {
            ...
            Ok((w, start, end)) => {
                ...
                let bounds_agree = end >= start && end - start == w.len() && text.get(start..end) == Some(w.as_str());
                if !(sub && within && bounds_agree) && over.is_none() { over = Some(json!({ "budget": budget, "window_len": w.len(), "is_substring": sub, "start": start, "end": end, "bounds_agree": bounds_agree })); }
            }
        }
    }
