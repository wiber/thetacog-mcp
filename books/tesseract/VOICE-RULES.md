...
### R08a — Restore the actor: jargon here is a deleted subject, not a hard word
The unreadable sentences in this book are almost never made of difficult ideas. They are made of **nominalizations** — a verb turned into a noun with the person doing it deleted. *Role continuity is attested at silicon.* Who is doing what? Nobody, to nothing. The reader cannot picture it because there is nobody in it, and a reader who cannot picture a sentence skims it.

The repair is mechanical and it takes one question: **who is doing this, and what are they doing?** Write that sentence.

- *"a model whose role continuity is attested at silicon"* → *"a model that has to keep proving it is still doing the job it was given"*
- *"contracts require continuous, accountable parties; continuous, accountable parties require role-continuity verification"* → *"the buyer cannot vouch for the buyer"*
- *"the divergence between them is the placement-pollution measurement"* → *"the gap between the two readings is the number you wanted"*

**When a sentence genuinely needs two parties, the transaction already has their names, and they are not Peter and Paul.** Invented first names are the wrong repair three times over: *rob Peter to pay Paul* is a live idiom in exactly the financial register this book works in, so the names arrive carrying a meaning nobody chose; a placeholder pair reads as a textbook exercise at the moment the book is claiming an Arm-and-Visa comp; and this book's whole credibility habit is that its examples are real — Fresno 1958, Pacioli 1494, CG 40 47, Lloyd's. **Buyer and supplier are names.** They happen to be jobs rather than first names, which is why they carry stakes a placeholder cannot.

Measure it: `node scripts/ops/jargon-density.mjs` ranks every prose sentence in the manuscript by abstract nouns per hundred words and prints the worst. It is advisory — a dense sentence is sometimes the right sentence — but a sentence in the top band is one you should have to defend.

### R09 — Canonical terms preserved exactly, and never asked to explain themselves
...
**And the half that was missing, which is what let R08a's failures through.** R09 protects a term where it is doing the job of a NAME. It does not license the term doing the job of an EXPLANATION, because **a category name cannot explain itself** — a reader who does not already hold the definition gets nothing from *"role continuity is attested at silicon."* So: **name it once, and show it working in the same breath.** The first appearance in a chapter carries the term AND the mechanism together — *"role continuity, read off silicon rather than asked for"* — and after that it has been earned and can stand alone. Paraphrasing the term away destroys the category, which is the failure R09 exists to stop; leaving it unaccompanied on first use destroys the sentence, which is the failure R09 was silent about.
