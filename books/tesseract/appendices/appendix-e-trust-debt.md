...
## 2. Deriving the 0.3%: The Original Per-Day Measurement

**Read the unit carefully, because this section is the one place in the book where the number wears a calendar.** Everywhere else, k_E = 0.003 is dimensionless and per boundary crossing. The ORIGINAL observation, recorded here unchanged, was a per-day drift measured on one PostgreSQL database over 365 days. The two coincide only because that database was crossed on roughly a daily cadence; the per-crossing form is the book's unit because it survives a system that crosses a thousand boundaries before lunch, and the per-day form does not. What follows is the provenance, not the definition.
...
**Average Daily Drift:**
D-bar = (1 / 365) SUM(i=1 to 365) D_i ~= -0.3%/day  [the original per-day form; one crossing per day in this dataset, so -0.3% per crossing]
...
- **Medium Churn (Enterprise SaaS):** 0.3% per boundary crossing, originally recorded as 0.3%/day at this tier's roughly daily commit cadence (annual drift: 30%)
