...
**The Critical Insight:** Evolution has invested **500 million years** optimizing neural information transfer, and the most specialised, highest-fidelity synapse anyone has put an electrode into still stops at 99.7%. That is the strongest **ceiling case** in the table above, and a ceiling case is exactly what it is — one exceptional synapse, measured, in one animal, not a bound proven to hold everywhere. It is enough. Five hundred million years of optimisation pressure failing to clear a number is worth a reader's attention without being promoted to a physical law, and nothing in this appendix rests on the promotion.
...
The five sections that follow (Sections 2-6) each start from a different branch of science and arrive at the same order of magnitude. What follows from that is smaller than it looks, and this appendix used to overstate it.

**Primary Claim:** k_E = 0.003 is a **convergence across independent derivations**, not a physical law, and the architecture does not rest on it.

**What that means, and what it does not.** Five lines of reasoning landing near one value is a striking result and we did not manufacture it. It is not a law, for the reason the [preface](/book/chapters/00-preface) gives about this same number in the book's own words -- *"This is NOT proven... observations from natural experiments, not controlled laboratory proofs"* -- and for a reason this appendix owes the reader plainly: the derivations below are not five measurements taken to a common protocol. They are five arguments, each starting from assumptions chosen by the person making it, and arguments that share an author's intuitions are not independent in the way that word usually promises. [Chapter 2](/book/chapters/02-universal-pattern-convergence) develops the same point, and the instruction there governs here: **say convergence; never say law.**

**And nothing downstream needs it to be a law.** The instrument counts boundary CROSSINGS against a declaration written before the run. The price per crossing is a separate question from whether the crossing happened, so if k_E were 0.004 in one substrate and 0.0028 in another, not one line of a receipt would change. Take this appendix apart and you have taken apart an interesting convergence, not the measurement.

The systems below each pay a cost of this order when S = P is violated (semantic does not equal physical). Read them as five observations that agree -- never as five proofs of one constant:
...
**Gap 4: "How does the Dark Silicon Semantic Scrubber avoid thermal runaway?"** It does add thermal load — switching a gate dissipates energy that leaving it dark does not, and any accounting that says otherwise is wrong. It avoids runaway by staying inside headroom that already exists: at 7nm and below, half to eighty percent of the die must sit unpowered at any instant to hold the TDP envelope, and combinational verification logic (no clock, no fetch, no register file) draws a fraction of what an arithmetic unit draws. The comparison that holds is against the BASELINE, never against idle: every eliminated DRAM access saves 100-400x the energy an L1 verification check spends, so the scrubber runs cooler than the cache-thrashing query it replaces. It does not run cooler than doing nothing. (Esmaeilzadeh et al., ISCA 2011.)
