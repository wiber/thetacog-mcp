...
This appendix derives, from cache-line mechanics, why database normalization **forces** pointer chasing and therefore cache-miss cascades. That derivation is the contribution and it stands on its own. The performance range it implies -- roughly 361x to 55,000x depending on whether the working set falls out to SSD or to disk -- is a **modelled estimate**, and this appendix used to call it something it is not.

**What this is not.** Earlier printings said "we prove," and cited "empirical measurement (production benchmarks)" and "hardware validation (CPU counter data)." **No benchmark in this repository produces 361x.** The only code that mentions the figure labels it a *speed claim* and stores it in a field named *potential*, which is the repo's own tooling being more careful than its book. Reading a modelled range as a measured one is the same error this book spends a chapter prosecuting elsewhere: a number given a provenance it has not earned.

**What survives, and it is the part that matters.** The mechanism is real and checkable without any multiple at all: a normalized query chases pointers, each hop is a potential cache miss, and a miss costs two orders of magnitude more than a hit. You can confirm that on your own machine with `pmu-onchip --boundary-probe`, which reports a median with its observed spread and refuses the run when the control band fails. **The argument needs the mechanism. It has never needed the multiple**, and if the range turns out to be 40x rather than 361x, not one claim in this book changes.
...
This is the hardware foundation for Chapter 1's claim that the checks are cheaper than the misses they replace. The xorALU does not just make verification cheap. It makes verification cheaper than the alternative of not verifying — because unverified data causes cache misses, and cache misses cost 100x what verification costs.
