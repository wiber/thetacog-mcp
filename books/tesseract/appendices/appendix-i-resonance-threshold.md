...
There is a subtraction a careful reader will perform here, so I will perform it first. The gain of sixteen is asserted from the block geometry -- sixteen blocks, sixteenfold amplification per bounce. Chapter six reports a different number: the walk on the real connectivity lattice measures a per-ply growth factor of about eleven and a half. These are different quantities -- gain per bounce of the cascade versus branching per ply of the definer walk -- and both can be right at once, but a hostile reader will not stop for the units. They will subtract sixteen from eleven and a half and bill the difference as an overclaim. So run this appendix's own formula on the weaker constant, the measured one: eleven and a half times one minus one over one hundred forty-four comes out to about 11.42. Still an order of magnitude past an ignition threshold of one. Every conclusion downstream of the crossover -- the supercriticality, the one-block ignition floor, the robustness margin -- survives on the measured number alone, borrowing nothing from the asserted geometry. From this paragraph forward, when I quote the margin, I quote the measured one. A claim that survives on its opponent's arithmetic is the strongest kind there is.
...
One honesty this derivation owes you before it goes further. It is an illustration, not a proof: a competent physicist will ask where eighty-three was measured, and the honest answer is that it was not -- it is a representative figure for the class of substrate, not a reading off any instrument. The constant does not lean on it. k_E = 0.003 stands as a calibrated per-crossing cost -- the patent's own claim language says it the same way, thresholds computed from the decay rate "determined during calibration" -- and the pre-registered half-life protocol, run against the 231-crossing prediction, is what will move it from calibrated to measured.
