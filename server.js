...
// C94b — THE EXTENSION LEAVES HOME (2026-09-19). The sidebar's sensors are scripts/vna/*.mjs, which exist only in the
// thetadrivencoach tree; in a stranger's repo the extension spawns `npx thetacog-mcp <door>` instead and this table answers.
// Each door is one script, bundled at prepack (bundle-pmu.mjs VNA_DOOR_SCRIPTS — the same list, one directory over from
// ENTRIES). The child keeps the CALLER's cwd and gets VNA_REPO = that cwd, so the page (steer-ui) is written into the
// stranger's repo where the extension's watcher reads it, and every receipt it cannot find prints `not run — npx thetacog-mcp
// <door>` (scripts/vna/door-cmd.mjs rewrites the page's own command lines away from home). A door whose script throws in a
// stranger's repo — an import that did not bundle, a receipt that is not there — prints `not run — <door>: <the error's own
// message>` on stderr, never a stack trace (THETACOG_TRACE=1 passes the trace through). The extension's mirror of this list
// is packages/thetacog-mcp-vscode/src/commands.ts; tests/vna/c94-foreign-repo-bootstrap.test.mjs diffs the two.
const VNA_DOORS = [
  ['steer-ui', 'scripts/vna/steer-ui.mjs'], ['cockpit', 'scripts/vna/cockpit.mjs'], ['aperture-receipt', 'scripts/vna/aperture-receipt.mjs'], ['recompute-receipt', 'scripts/vna/recompute-receipt.mjs'], ['story', 'scripts/vna/story.mjs'],
  ['alignment-map', 'scripts/vna/alignment-map.mjs'], ['competence-pixel', 'scripts/vna/competence-pixel.mjs'], ['parametric-trigger', 'scripts/vna/parametric-trigger.mjs'],
  ['envelope', 'scripts/vna/envelope.mjs'], ['mesh', 'scripts/vna/mesh.mjs'], ['spec-tree', 'scripts/vna/spec-tree.mjs'],
  ['goal', 'scripts/vna/goal.mjs'], ['runner-reading', 'scripts/vna/runner-reading.mjs'], ['straight-path', 'scripts/vna/straight-path.mjs'],
  ['clip-ingest', 'scripts/vna/clip-ingest.mjs'], ['clip-watch', 'scripts/vna/clip-watch.mjs'], ['underwriter-line', 'scripts/vna/underwriter-line.mjs'],
  ['auth-flow', 'scripts/vna/auth-flow.mjs'],
  // C297 — the 💬 Steer Chat's own doors (the extension's mirror: packages/thetacog-mcp-vscode/src/commands.ts DOORS)
  ['steer', 'scripts/vna/steer.mjs'], ['workers', 'scripts/vna/workers.mjs'], ['keep-running', 'scripts/vna/keep-running.mjs'],
  ['chat-context', 'scripts/vna/chat-context.mjs'], ['chat-model-verb', 'scripts/vna/chat-model-verb.mjs'], ['steer-runner', 'scripts/vna/steer-runner.mjs'],
  ['runner-resolver', 'scripts/vna/runner-resolver.mjs'], ['local-models', 'scripts/vna/local-models.mjs'],
  // C309d — the buyer's twelve-state monologue: `npx thetacog-mcp buyer-monologue [--signing] [--outsider "<name>"]`
  ['buyer-monologue', 'scripts/vna/buyer-monologue.mjs'],
];
const VNA_DOOR_CMDS = new Set(VNA_DOORS.map(([c]) => c));
const KNOWN_DISPATCH_CMDS = [];
// C97c: `verify <url>` — an http(s) argument — is the CAR-by-URL verifier (scripts/pmu/verify-url.mjs: fetch, recompute every row
// hash, signature, MMR proof and countersignature against <origin>/api/auth/keys, nothing read from disk); `verify <row.json>`
// stays verify-row.mjs. The child needs no cwd: the URL is the whole input.
if (process.argv[2] === 'verify' && /^https?:\/\//.test(process.argv[3] || '')) process.argv[2] = 'verify-url';
for (const [cmd, rel] of [['verify', 'scripts/pmu/verify-row.mjs'], ['verify-url', 'scripts/pmu/verify-url.mjs'], ['attest-bundle', 'scripts/pmu/attest-bundle.mjs'], ['prove', 'scripts/pmu/prove.mjs'], ['sense', 'scripts/pmu/sense.mjs'], ['attest', 'scripts/pmu/attest.mjs'], ['prove-rice', 'scripts/pmu/prove-rice.mjs'], ['hooper', 'scripts/pmu/hooper.mjs'], ['price-attest', 'scripts/pmu/price-attest.mjs'], ['attest-demo', 'scripts/pmu/attest-demo.mjs'], ['hypotheses', 'scripts/pmu/attest-hypotheses.mjs'], ['ur-attest', 'scripts/pmu/ur-attest.mjs'], ['perturb', 'scripts/pmu/attest-perturb.mjs'], ['attest-open', 'scripts/pmu/attest-open.mjs'], ['lens-tester', 'scripts/pmu/lens-tester-open.mjs'], ['lens-run', 'scripts/pmu/lens-run.mjs'], ['dinner-demo', 'scripts/pmu/dinner-demo.mjs'], ['publish-commit', 'scripts/pmu/publish-commit.mjs'], ['bootstrap', 'scripts/pmu/bootstrap.mjs'], ['proof', 'scripts/pmu/proof-monologue.mjs'], ['confidence', 'scripts/pmu/confidence-overlay.mjs'], ['annotate', 'scripts/pmu/annotate-regions.mjs'], ['portfolio', 'scripts/pmu/project-portfolio.mjs'], ['underwriter-grade', 'scripts/pmu/underwriter-grade.mjs'], ['settle', 'scripts/pmu/onchain-settle.mjs'], ['premium', 'scripts/pmu/calibration-premium.mjs'], ['ledger-attest', 'scripts/pmu/ledger-attest.mjs'], ['variance', 'scripts/pmu/variance-option.mjs'], ['anchor', 'scripts/pmu/onchain-anchor.mjs'], ['mesh-up', 'scripts/mesh/mesh-up.mjs'], ['spec-drift', 'scripts/pmu/spec-drift-check.mjs'], ['intervene', 'scripts/pmu/intervene.mjs'], ['e0', 'scripts/pmu/lens-e0.mjs'], ['miner', 'scripts/pmu/reef-miner.mjs'], ['harvest', 'scripts/pmu/harvest-agent.mjs'], ['sweep', 'scripts/pmu/lens-convergence-sweep.mjs'], ['loop-story', 'scripts/pmu/reef-loop-story.mjs'], ['register-surface', 'scripts/pmu/register-surface.mjs'], ['verify-baseline', 'scripts/pmu/verify-baseline.mjs'], ['init-fork', 'scripts/pmu/init-fork.mjs'], ['simulate-payout', 'scripts/pmu/simulate-payout.mjs'], ['my-pixel', 'scripts/pmu/my-pixel.mjs'], ['map-crawl', 'scripts/pmu/map-crawl.mjs'], ['verify-peer', 'scripts/pmu/verify-peer.mjs'], ['big-map-axes', 'scripts/pmu/big-map-axes.mjs'], ['index', 'scripts/pmu/lane-index.mjs'], ['vega-backtest', 'scripts/pmu/vega-backtest.mjs'], ['advisory-premium', 'scripts/pmu/advisory-premium.mjs'], ['trigger-battery', 'scripts/pmu/trigger-battery.mjs'], ['walk', 'scripts/pmu/walk.mjs'], ['replay', 'scripts/pmu/replay.mjs'], ...VNA_DOORS]) {
  ...
  if (process.argv[2] === cmd) {
    ...
    // `intervene`, `walk` (C92d, the one-minute door) and `replay` (C99d, the door over a range) measure the CALLER'S repo
    // (REPO = process.cwd()) — preserve cwd even when the script itself resolves from the bundled package (a stranger's repo has no scripts/pmu/).
    // …and every vna door (C94b): the caller's cwd is the repo being read, VNA_REPO names it for the scripts that honour it.
    const vna = VNA_DOOR_CMDS.has(cmd);
    const childCwd = (cmd === 'intervene' || cmd === 'walk' || cmd === 'replay' || vna) ? process.cwd() : root;
    const env = vna ? { ...process.env, VNA_REPO: process.env.VNA_REPO || process.cwd() } : process.env;
    const r = spawnSync('node', [path.join(root, rel), ...process.argv.slice(3)], { stdio: vna && !process.env.THETACOG_TRACE ? ['inherit', 'inherit', 'pipe'] : 'inherit', cwd: childCwd, env });
    if (vna && !process.env.THETACOG_TRACE) {
      const err = String(r.stderr || '');
      const stackAt = err.search(/\n\s+at [\w.<> ]+\(?[^\n]*:\d+:\d+\)?/);
      if ((r.status ?? 0) !== 0 && stackAt >= 0) {
        // the error's own first line, never the trace: a stranger's card reads a reason, not our file paths
        const lines = err.slice(0, stackAt).trim().split('\n').filter(Boolean);
        const msg = (lines.filter((l) => /Error|error|Cannot|cannot/.test(l)).pop() || lines.pop() || 'the door threw').replace(/^\S*[\/\\]/, '').trim();
        process.stderr.write(`not run — ${cmd}: ${msg.slice(0, 300)}\n`);
