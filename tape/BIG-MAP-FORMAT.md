...
from events + crawl, and disputes resolve by recomputation from the mounted surface.

**What ships, said exactly (corrected 2026-08-25 — the earlier "the transport already ships
locally" read as if these four kinds were accepted; they are not).** The transport ships for
the WORK lifecycle and rejects every kind outside it, so a `mount` event posted today throws.

SHIPPED KINDS (accepted by `scripts/mesh/mesh-post.mjs` today): `ASK` · `CLAIM` · `HEARTBEAT` ·
`VERIFY_PAYLOAD` · `VERDICT` · `CLOSED` · `REOPENED` · `ESCALATED`

DEFERRED KINDS (named by this format, NOT yet accepted by the transport): `MOUNT` · `UPDATE` ·
`ATTEST` · `UNMOUNT`

Shipping one of the deferred kinds means adding it to the transport's `TYPES` **and** giving it
a projection case in `mesh-replay.mjs` in the same commit — the replay fold currently has no
`default`, so an unrecognised kind is dropped from the projection silently rather than refused.
Until then the map view is regenerated from the CRAWL alone, which is why nothing on the map
depends on these events being live. Guarded by `tests/mesh/big-map-grammar-honesty.test.mjs`;
specced as G1/R1–R2 in `docs/specs/drafts/architect-reach-is-verify-map-spec-v2.md`.
