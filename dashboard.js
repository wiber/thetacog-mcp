...
// The Next.js app (tesseract, /commit receipts, /insurability) is a SEPARATE
// surface on its own port — the dashboard links out to it, it does not proxy it
// (the /page/ route is hard-fenced to .workflow + cognitive-workspace files).
const APP_PORT = parseInt(process.env.THETACOG_APP_PORT || '3000', 10);
const APP_ORIGIN = process.env.THETACOG_APP_ORIGIN || `http://127.0.0.1:${APP_PORT}`;
...
// Live surfaces on the app port, surfaced in the dashboard nav under "app".
// The tesseract is NOT part of this dashboard's page tree — it is a Next.js
// route on APP_PORT — so the only honest integration is an outbound link.
// EVERY ENTRY MUST BE A REAL src/app ROUTE. This list is hand-maintained and the
// dashboard cannot see the Next router, so it can drift into dead links silently —
// which is worse than no link, because a dead nav entry reads as a broken product.
// Guarded by tests/rooms/dashboard-app-surfaces.test.mjs: each route must resolve to
// a page.tsx (or a route group / dynamic segment) under src/app.
export const APP_SURFACES = [
  // Tesseract GUI — the full set of static surfaces, all verified 200 on 2026-08-11.
  { route: '/tesseract', title: '🧊 Tesseract' },
  { route: '/tesseract/grid', title: '🧊 Tesseract · grid' },
  { route: '/tesseract/feed', title: '🧊 Tesseract · feed' },
  { route: '/tesseract/leaderboard', title: '🧊 Tesseract · leaderboard' },
  { route: '/tesseract/spectator', title: '🧊 Tesseract · spectator' },
  { route: '/tesseract/me', title: '🧊 Tesseract · me' },
  { route: '/tesseract/login', title: '🧊 Tesseract · login' },
  { route: '/commit', title: '🧾 Commit receipts' },
  { route: '/insurability', title: '📈 Insurability' },
  { route: '/pixel', title: '🎯 Pixel' },
  { route: '/rooms', title: '🚪 Rooms' },
];

// Measured liveness of the app port (30s cache) — a dead link is dimmed in the
// nav rather than silently pretending the surface is there.
// A NEGATIVE is cached for only 3s: the first probe can race the app's own
// startup, and a 30s-sticky false would dim every app link on first load.
let _appAlive = { at: 0, up: false };
function appPortAlive() {
  const ttl = _appAlive.up ? 30_000 : 3_000;
  if (_appAlive.at && Date.now() - _appAlive.at < ttl) return Promise.resolve(_appAlive.up);
  return new Promise((resolve) => {
    const sock = net.connect({ port: APP_PORT, host: '127.0.0.1' });
    const done = (up) => {
      sock.destroy();
      _appAlive = { at: Date.now(), up };
      resolve(up);
    };
    sock.setTimeout(250);
      ...
      Object.entries(byCat).map(([cat, ps]) =>
        ...
        ps.map(p => p.url
          ? '<a href="' + escape(p.url) + '" target="_blank" rel="noopener"'
              + (p.up === false ? ' style="opacity:.45" title="' + escape(p.url) + ' — app not running"' : ' title="' + escape(p.url) + '"')
              + '>' + escape(p.title || p.file) + (p.up === false ? ' ·off' : ' ↗') + '</a>'
          : '<a href="/page/' + encodeURIComponent(p.path) + '" title="' + escape(p.path) + '">' + escape(p.title || p.file) + '</a>').join('')
      ).join('');
...
`;
    ...
    if (p === '/api/pages' && req.method === 'GET') {
      ...
      // App surfaces: absolute `url`, never `path` — these live on the Next.js
      // port, so routing them through /page/ would 400 (forbidden path).
      // MEASURED, not asserted: `up` comes from a real TCP connect, cached 30s.
      const up = await appPortAlive();
      for (const s of APP_SURFACES) {
        pages.push({ category: 'app', file: s.route, url: APP_ORIGIN + s.route, title: s.title, up });
      }
